...
//    private int customerId;
    ...
    private String consultantCaptchaText;
 ...
 *
 * ForUse : captchaValidation() is used for validate google recaptcha.
 * 
 *
 * *****************************************************************************
 */
 ...
 * Author : Rama Rao Penugonda<rpenugonda@miraclesoft.com>
 *
 * ForUse : getCaptchaImage() is used for getting simple captcha image.
 * 
 *
 * *****************************************************************************
 */
...
public void getCaptchaImage()throws Exception{
	//System.out.println("captcha flag value is "+getCaptchaFlag());
    ...
    */
    ...
    fonts.add(new Font("Arial", Font.HANGING_BASELINE, 35));
     
    Captcha captcha = new Captcha.Builder(110, 55)
    ...
    System.out.println("captcha flag in captcha image"+getCaptchaFlag());
    // save the captcha value on session
    if(getCaptchaFlag().equalsIgnoreCase("CVReg"))
    {
    	httpServletRequest.getSession(false).setAttribute("CVCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("custLoginCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("custLoginCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("venLoginCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("venLoginCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("conLoginCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("conLoginCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("csrLoginCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("csrLoginCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("conCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("conCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("CVFPCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("CVFPCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("conFPCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("conFPCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("conLoginRegCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("conLoginRegCaptchaSes",captcha.getAnswer());
    }
    else if(getCaptchaFlag().equalsIgnoreCase("conFPRegCaptcha"))
    {
    	httpServletRequest.getSession(false).setAttribute("conFPRegCaptchaSes",captcha.getAnswer());
    }
 ...
 * Author : Rama Rao Penugonda<rpenugonda@miraclesoft.com>
 *
 * ForUse : simpleCaptchaValidation() is used for validate the captcha.
 * 
 *
 * *****************************************************************************
 */

public String simpleCaptchaValidation() {
	
    System.out.println("-------In  simpleCaptchaValidation class-------");
    resultMessage = LOGIN;
    boolean result;
   String captchaSessionValue=null;
   System.out.println("captcha flag value in validation "+getCaptchaFlag());
  //  if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID) != null) {
        try {
        	if(getCaptchaFlag().equalsIgnoreCase("CVReg"))
            {
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("CVCaptchaSes").toString();
        	
            }
        	else if(getCaptchaFlag().equalsIgnoreCase("custLoginCaptcha"))
            {
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("custLoginCaptchaSes").toString();
            }
        	else if(getCaptchaFlag().equalsIgnoreCase("venLoginCaptcha"))
            {
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("venLoginCaptchaSes").toString();
            }
        	else if(getCaptchaFlag().equalsIgnoreCase("conLoginCaptcha"))
            {
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("conLoginCaptchaSes").toString();
            }
        	else if(getCaptchaFlag().equalsIgnoreCase("csrLoginCaptcha"))
            {
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("csrLoginCaptchaSes").toString();
            }
        	
        	else if(getCaptchaFlag().equalsIgnoreCase("conCaptcha"))
        	{
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("conCaptchaSes").toString();
        	}
        	else if(getCaptchaFlag().equalsIgnoreCase("CVFPCaptcha"))
        	{
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("CVFPCaptchaSes").toString();
        	}
        	else if(getCaptchaFlag().equalsIgnoreCase("conFPCaptcha"))
        	{
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("conFPCaptchaSes").toString();
        	}
        	else if(getCaptchaFlag().equalsIgnoreCase("conLoginRegCaptcha"))
        	{
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("conLoginRegCaptchaSes").toString();
        	}
        	else if(getCaptchaFlag().equalsIgnoreCase("conFPRegCaptcha"))
        	{
        	captchaSessionValue=httpServletRequest.getSession(false).getAttribute("conFPRegCaptchaSes").toString();
        	}
        	
        		
        	System.out.println("captcha session value"+captchaSessionValue);
        	System.out.println("captcha value from html"+getConsultantCaptchaText());
        	
        	if(captchaSessionValue.equals(getConsultantCaptchaText()))
        	{
        		result=true;
        	}
        	else
...
public void setServletRequest(HttpServletRequest httpServletRequest) {
	...
	public String getConsultantCaptchaText() {
		return consultantCaptchaText;
	}

	public void setConsultantCaptchaText(String consultantCaptchaText) {
		this.consultantCaptchaText = consultantCaptchaText;
	}
