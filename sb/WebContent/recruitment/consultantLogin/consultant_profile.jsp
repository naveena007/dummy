...
<script>
	...
	function doOnLoad() {
		...
		if (flag == '11') {
			document.getElementById('csrstatus').disabled = true;
			document.getElementById('csrComments').disabled = true;
			document.getElementById('csrButton').style.display = "none";
		}
					...
					<div class="row">
						...
						<div class="col-sm-12 col-md-9 col-lg-9 right_content"
							...
							<!-- Nav tabs -->
								...
								<s:if test="%{#session.primaryrole!=15||#session.primaryrole!=16}"><s:hidden name="conrequirementId"
												id="conrequirementId" value="%{requirementId}" /></s:if>
							...
							<ul class="nav nav-tabs">
								<li class="active" id="consultantLi"><a
								 ...
								 <s:if test="%{#session.primaryrole==15||#session.primaryrole==16 ||#session.primaryrole==8 ||#session.primaryrole==9 ||#session.primaryrole==10 ||#session.primaryrole==11 ||#session.primaryrole ==0 ||#session.primaryrole==1}"> 		
								...
								<li id="backgroundVerificationTab" class=""><a
									...
									onclick="showBackGroundAttachmentDetails(document.getElementById('consult_id').value,<%=session.getAttribute("primaryrole")%>);">
								...
								<s:elseif test="%{pageFlag=='submitList' && vendor=='yes'}">
								<s:url var="myUrl"
									            action="../../Requirements/getSubmissionList.action">
									           <s:param name="orgid"><s:property value="%{accountSearchID}"/></s:param>
									            <s:param name="pageFlag">submitList</s:param>
								...
								<s:if test="pageFlag=='profileForVendorCustomer'&& vendor=='yes'">
								 ...
								 <s:if test="consultantAddFlag=='yes'&& vendor=='yes'">
													<s:url var="myUrl"
														action="../../recruitment/consultant/doAddConsultantForReq.action">
														<s:param name="jdId"><s:property value="%{jdId}"/></s:param>
														<s:param name="vendor"><s:property value="%{vendor}"/></s:param>
														<s:param name="requirementId" value="%{requirementId}" ></s:param>
														<s:param name="jobTitle" value="{jobTitle}"></s:param> 
														<s:param name="targetRate" value="{targetRate}"></s:param>
														<s:param name="maxRate" value="{maxRate}"></s:param>
														<s:param name="reqSkill" value="{reqSkill}"></s:param>
														<s:param name="orgid" value="{orgid}"></s:param>
													</s:url>
								</s:if> 
									...
									<div class="panel-body">
										...
										<form action="updateConsultantProfile" id="consultantForm"
											...
											<s:hidden value="%{ConsultantVTO.consult_id}"
												name="consult_id" />
										    <s:hidden value="%{requirementId}"
												name="requirementId" />
											<s:hidden value="%{accountFlag}"
												name="accountFlag" />
											<s:hidden value="%{accountSearchID}"
												name="accountSearchID" />
											<s:hidden value="%{jdId}"
												name="jdId" />
											<s:hidden value="%{pageFlag}"
												name="pageFlag" />
											<s:hidden value="%{vendor}"
												name="vendor" />
											<s:hidden value="%{customerFlag}"
												name="customerFlag" />
											<s:hidden value="%{vendorcomments}"
												name="vendorcomments" />
											<s:hidden value="%{consultStatus}"
												name="consultStatus" />
											<s:hidden value="%{orgid}"
												name="orgid" />
											<s:hidden value="%{targetRate}"
												name="targetRate" />
											<s:hidden value="%{maxRate}"
												name="maxRate" />
											<s:hidden value="%{reqSkill}"
												name="reqSkill" id="reqSkill"/>
											<s:hidden value="%{jobTitle}"
												name="jobTitle" id="jobTitle"/>
										    <s:hidden id="consultantAddFlag" name="consultantAddFlag" value="%{consultantAddFlag}"/>
...
<%-- 
									...
									%> --%>
									<s:if test="%{#session.primaryrole==1 || #session['usrgrpid']==1}">
									<br>
									...
									<span class="required">
										<label class="labelStylereq" style="color: #56a5ec;">
											CSR Status </label></span>
										<s:select name="csrstatus" id="csrstatus"
											value="%{consultantVTO.csrAppStatus}" tabindex="2"
											headerKey="-1" headerValue="Please select"
											list="#@java.util.LinkedHashMap@{'Approve':'Submit to Customer','Reject':'Reject'}"
											cssClass="SelectBoxStyles form-control" />
									</div>
									...
									<div class="col-sm-2 pull-right" style="margin-top: 65px">
										<button   id="csrButton"
											...
											onclick="updateCSRStatus();"><i class="fa fa-refresh"></i>&nbsp;Update</button>
          ...
          <h4 class="modal-title">Modal Header</h4>
											...
											<div class="modal-body">
												<s:form action="consultBackgroundUpload"
													...
													<s:hidden name="current_attachmentId"
														id="current_attachmentId" />	
										...
										<div class="col-lg-12">
											...
											<table id="consult_background_attacment_table"
												...
												<s:if test="%{#session.primaryrole==15||#session.primaryrole==16}">
													...
													<th>Attachment Type</th>
													<!--<th>Location</th>-->
													<th>Form Initiated Date</th>
													<th>Download Link</th>
													<th>Status</th>
													<s:if test="%{#session.primaryrole==15||#session.primaryrole==16}">
													<th>Upload/Download ScannedCopy</th>
													</s:if>
													<s:else>
													<th>Download Consultant Uploaded File</th>
													</s:else>
													<th>Consultant Uploaded Date</th>
													<th>Background Status</th></tr>
											</table>
											<!--  <br> <label> Display <select
												id="con_paginationOption" class="disPlayRecordsCss"
												onchange="con_pagerOpt	ion()" style="width: auto">
													<option>10</option>
													<option>15</option>
													<option>25</option>
													<option>50</option>
      ...
      <div class="backgroundcolor">
        ...
        <div class="modal-body">
         <s:form action="/Requirements/consultBackgroundStatusUpdate"
												...
												<s:hidden name="backgroundStatus" id="CSRBackgroundStatus" value="Verified"/>	
													<div class="inner-addtaskdiv-elements">
