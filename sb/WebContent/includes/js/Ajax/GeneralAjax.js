...
function forgotPassword(emailId, resultMessage, loadingMessage) {
	...
	if (ValidateEmailForgotPwd(email)) {
		...
		$('#' + loadingMessage).hide();
		$("#" + resultMessage).html(
				"<font color=red><br>Please enter valid e-mail id</font>");
		setTimeout(function() {
			$("#" + resultMessage).children().remove();
		}, 3000);
...
function setPaybleDetails(){
	...
	if (invoiceStatus == 'Approved') {
		...
		document.getElementById("invoicePaid").style.display = "none";
	}
	else if (invoiceStatus == 'Paid') {
		document.getElementById("paymentData").style.display = "block";
		document.getElementById("invoicePaid").style.display = "block";
		return true;
	}
	else{
...
/*function payableValidation(){
	var invoiceStatus = document.getElementById("invoiceStatus").value;
    var pamentDate=document.getElementById("pamentDate").value;
	 //alert("set payable details onchange"+invoiceStatus);
	if (invoiceStatus == 'Approved') {
		if (pamentDate == "" || pamentDate == null) {
			$("payError")
					.html(
							" <font color=#FF4D4D> PayableDate field is Required</font>.");
			$("#pamentDate").css("border", "1px solid red");
			return false;
		} else {
			$("#pamentDate").css("border", "1px solid #3BB9FF");
			$("payError").html("");
			return true;
		}
	}
...
function conForgotPassword(emailId, resultMessage, loadingMessage) {
	...
	captchaValue=document.getElementById("conFPCaptchaText").value;
	res=simpleCaptchaValidation(captchaValue,'conFPCaptcha');
	
	if (res === true) {
		forgotPassword(emailId, resultMessage, loadingMessage);
	} else {
		$("#conFPCaptchaText").val("");
		loadNewCaptcha('conFPCaptcha');
		$("#" + resultMessage).html(
				"<font color=red size='3px'><br>Invalid captcha..</font>");
		setTimeout(function() {
			$("#" + resultMessage).children().remove();
		}, 3000);
		return false;
...
/*	response = custVenForgotPasswordRecaptchaValidation();*/
	captchaValue=document.getElementById("CVFPCaptchaText").value;
	response=simpleCaptchaValidation(captchaValue,'CVFPCaptcha');
	// alert(response);
	if (response === true) {
		forgotPassword(emailId, resultMessage, loadingMessage);
	} else {
		$("#CVFPCaptchaText").val("");
		loadNewCaptcha('CVFPCaptcha');
		$("#" + resultMessage).html(
				"<font color='red' size='1'><br>Invalid captcha..</font>");
		setTimeout(function() {
			$("#" + resultMessage).children().remove();
		}, 3000);
...
function conFpassInReg(emailId, resultMessage, loadingMessage) {
	var response = false;
	captchaValue=document.getElementById("conFPRegCaptchaText").value;
	response = simpleCaptchaValidation(captchaValue,'conFPRegCaptcha');
	// alert(response);
	if (response === true) {
		forgotPasswordLink(emailId, resultMessage, loadingMessage);
	} else {
		$("#conFPRegCaptchaText").val("");
		loadNewCaptcha('conFPRegCaptcha');
		$("#" + resultMessage).html(
			...
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					...
					if (req.status == 200) {
						...
						// window.location = "getAccAuthrization.action?resultMessage="+
						if(req.responseText=="RateCard Already Existed"){
							document.getElementById("overlayJobTitle").value="";
							$('#validationMessage').html("<font color='red'>"+ req.responseText +"</font>").show().delay(3000).fadeOut();
							if(flag==0){
							add.style.display = "block";
							}
						}
						else{
						$('#validationMessage').html("<font color='green'>"+ req.responseText +"</font>").show().delay(3000).fadeOut();
...
function setSecurityDepositValues(securityDepositId,securityDepositTitle,profileSelectionCost,profileSubmissionCost, profileRejectionCost,fakeCandidateFine, comments) {
	// alert(id + "" + jobTitle + "" + minRate + "" + maxRate + "" + comments);
	document.getElementById("overlaySecurityDepositId").value = securityDepositId;
	document.getElementById("overlaySecurityDepositTitle").value = securityDepositTitle;
	//document.getElementById("overlayDepositAmount").value = depositAmount;
	document.getElementById("overlayProfileSubmissionCost").value = profileSubmissionCost;
	document.getElementById("overlayProfileSelectionCost").value = profileSelectionCost;
	
	document.getElementById("overlayProfileRejectionCost").value = profileRejectionCost;
	document.getElementById("overlayFakeCandidateFine").value = fakeCandidateFine;
	document.getElementById("overlayComments").value = comments;
	/*document.getElementById("overlayVendorTiers").value = vendorTiers;
	document.getElementById("overlayFromDate").value = fromDate;
	document.getElementById("overlayToDate").value = toDate;*/
}
function addOrUpdateSecurityChecking(flag) {
	// alert(flag);
	if (flag == 'a') {
		// alert("add");
		var update = document.getElementById("updateDiv");
		...
		document.getElementById("overlayHeading").innerHTML = "&nbsp;&nbsp;Add&nbsp;Security&nbsp;Deposit&nbsp;Policy";
	}
	if (flag == 'u') {
		// alert("Updatew");
		var add = document.getElementById("addDiv");
		...
		document.getElementById("overlayHeading").innerHTML = "&nbsp;&nbsp;Update&nbsp;Security&nbsp;Deposit&nbsp;Policy";

	}
}
function clearSecurityDepositValuesForAdd() { 
	// alert("clearActionValues");
	document.getElementById("overlaySecurityDepositId").value = "";
	document.getElementById("overlaySecurityDepositTitle").value = "";
	//document.getElementById("overlayDepositAmount").value = "";
	document.getElementById("overlayProfileSubmissionCost").value = "";
	document.getElementById("overlayProfileSelectionCost").value = "";
	document.getElementById("overlayProfileRejectionCost").value = "";
	document.getElementById("overlayFakeCandidateFine").value = "";
	document.getElementById("overlayComments").value = "";
	/*document.getElementById("overlayVendorTiers").value = "-1";
	document.getElementById("overlayFromDate").value = "";
	document.getElementById("overlayToDate").value = "";*/
	var add = document.getElementById("addDiv");
	add.style.display = "block";

}
function insertOrUpdateSecurityDeposit(flag) {
	
	
	var overlaySecurityDepositId = document.getElementById("overlaySecurityDepositId").value;
	var overlaySecurityDepositTitle = document.getElementById("overlaySecurityDepositTitle").value;
	
	//var overlayDepositAmount = document.getElementById("overlayDepositAmount").value;
	var overlayProfileSubmissionCost = document.getElementById("overlayProfileSubmissionCost").value;
	var overlayProfileSelectionCost = document.getElementById("overlayProfileSelectionCost").value;	
	var overlayProfileRejectionCost = document.getElementById("overlayProfileRejectionCost").value;
	var overlayFakeCandidateFine = document.getElementById("overlayFakeCandidateFine").value;
	var overlayComments = document.getElementById("overlayComments").value;
	/*var overlayVendorTiers=document.getElementById("overlayVendorTiers").value;
	var overlayFromDate=document.getElementById("overlayFromDate").value;
	var overlayToDate=document.getElementById("overlayToDate").value;*/
	// var status=document.getElementById("status").value;
	//alert(overlayRatecardId);
	// alert(accauthStatus);
	// alert(addingAccAuthDesc);
	// alert(actionId);
	/*if(overlayDepositAmount.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Deposit Amount</font>";
		return false;
	}*/
	if(overlaySecurityDepositTitle.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Security Deposit Title</font>";
		return false;
	}
	if(overlayProfileSubmissionCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Submission Cost</font>";
		return false;
	}
	if(overlayProfileSelectionCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Selection Cost</font>";
		return false;
	}
	if(overlayProfileRejectionCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Rejection Cost</font>";
		return false;
	}
	if(overlayFakeCandidateFine.trim()==""){
	...
	/*if(overlayVendorTiers=="-1"){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Select Vendor Tiers</font>";
		return false;
	}
	if(overlayFromDate==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter From Date</font>";
		return false;
	}
	if(overlayToDate==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter To Date</font>";
		return false;
	}*/
	else{
	document.getElementById("loadingSecurityDepositSearch").style.display="block";
	initSessionTimer();
	...
	/*var url = CONTENXT_PATH
			+ '/Requirements/insertOrUpdateSecurityDeposit.action?securityDepositId='
			+ overlaySecurityDepositId + '&depositAmount=' + overlayDepositAmount + '&profileSubmissionCost='
			+ overlayProfileSubmissionCost + '&profileRejectionCost=' + overlayProfileRejectionCost + '&fakeCandidateFine=' + overlayFakeCandidateFine + '&comments='
			+ overlayComments+ '&vendorTiers=' + overlayVendorTiers+ '&fromDate=' + overlayFromDate+ '&toDate=' + overlayToDate;

	*/
	
	var url = CONTENXT_PATH
	+ '/Requirements/insertOrUpdateSecurityDeposit.action?securityDepositId='
	+ overlaySecurityDepositId + '&profileSelectionCost=' + overlayProfileSelectionCost + '&securityDepositTitle=' + overlaySecurityDepositTitle + '&profileSubmissionCost='
	+ overlayProfileSubmissionCost + '&profileRejectionCost=' + overlayProfileRejectionCost + '&fakeCandidateFine=' + overlayFakeCandidateFine + '&comments='
	+ overlayComments;  
	...
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
			...
			if (req.status == 200) {
				...
				// document.getElementById("validationMessage").innerHTML=req.responseText;
				// window.location = "getAccAuthrization.action?resultMessage="+
				 $('#validationMessage').html(" <font color='green'>" + req.responseText +"</font>").show().delay(4000).fadeOut();;
...
function reloadSecurityDepositPage(){
	window.location = CONTENXT_PATH+"/Requirements/getSecurityDepositList.action";
}

function descriptionCheckCharacters(id){
    $(id).keyup(function(){
        el = $(this);
        e2 = $(this);
        var count = el.val().length + ((el.val().split('\n').length)*5);
        if(count >= 200){
            e2.val( e2.val().substr(0, 200-((el.val().split('\n').length)*5)) );
        } else {
            $("#descriptionCharNum").text(200-count+' Characters remaining . ');
        }
        if(count==200)
        {
            $("#descriptionCharNum").text(' Cannot enter  more than 200 Characters .');    
        }
        
    })
    return false;
};
function simpleCaptchaValidation(captchaValue,flag)
{
 //alert(captchaValue);
	//alert("flag value is "+flag);
	var url = CONTENXT_PATH + "/simpleCaptchaValidation.action?consultantCaptchaText="
			+ captchaValue+ '&captchaFlag=' + flag;
	// alert("url-->"+url);
	var req = initRequest(url);
	req.onreadystatechange = function() {
...
function loadNewCaptcha(flag)
{ 
	//alert(flag);
	//document.getElementById(flag).src="../users/general/getCaptchaImage?t=t"+ Math.random(5)+'&captchaFlag=' + flag;
	var url="/sb/users/general/getCaptchaImage?captchaFlag=" + flag+"&timestamp=" + new Date().getTime();
	//alert(url);
	$("#"+flag).attr("src",url);
}
function getSearchCostCenterList(element)
{
   ...
   //alert(CONTENXT_PATH);
   var url = CONTENXT_PATH+'/costCenter/costCenterBySearch.action?ccName=' + ccName + '&ccCode=' + ccCode + '&status=' + status + '&pgFlag='+pGflag+'&pgNo='+pgNo+'&recordPage='+recordPage;
    
	var req = initRequest(url);
    req.onreadystatechange = function() {
       document.getElementById('loadingCostCenterSearch').style.display = 'block';
        if (req.readyState == 4 && req.status == 200) {
      $('#loadingCostCenterSearch').hide();
            // alert("response"+req.responseText);
            populateCostCenterTableReq(req.responseText,element);
        }
    };
    req.open("GET", url, "true");
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function populateCostCenterTableReq(response,flag) {
    //alert(response);
	//alert(flag);
    $(".page_option").css('display', 'block');
    var dataArray = response.split("###");
    var ccList = dataArray[0].split("^");
	var roleValue=document.getElementById("role").value;
	//alert(roleValue);
    var table = document.getElementById("costCenterResults");
    ...
    if(flag=='1'){
        //alert("total records are "+dataArray[1]);
          ...
          document.getElementById("ccTotalRecords").value=dataArray[1];
      }
    //alert("record pages are "+recordPage);
    var urlLink,aLink;
    if ((document.getElementById("ccTotalRecords").value) > 0) {
        for (var i = 0; i < ccList.length - 1; i++) {

            var Values = ccList[i].split("|");
            {
            	urlLink="costCenterInfo.action?ccCode="+Values[1]+"&ccName="+Values[2];
                var reqRow = $("<tr />")
                $("#costCenterResults").append(reqRow); //this will append tr element to table... keep its reference for a while since we will add cels into it
                	
					reqRow.append($('<td><a href="#" class="costCenter_popup_open" onclick="costCenter_overlay('+Values[0]+',\''+Values[2]+'\','+Values[4]+');">'+Values[1]+"</td>"));	
					...
					if(Values[4]>0)
					{
					reqRow.append($('<td><a href="#" class="costCenterProjects_popup_open" onclick="costCenter_projects(\''+Values[1]+'\');">'+Values[4]+"</td>"));
					}
					else
					{
					...
					//alert(Values[6]);
				if(Values[6]==0||Values[6]==null)
				{
					Values[6]=0;
				if(roleValue!='Director')
				{
				
				reqRow.append($('<td><a href="#" class="costCenterBudget_popup_open" onclick="costCenterBudget_overlay(\''+Values[1]+'\','+Values[6]+',\''+Values[2]+'\');">'+"<i class='fa fa-plus-square'></i></td>"));
				
				}
				else{
				reqRow.append($('<td> <i class="fa fa-plus-square"></i> </td>'));
				}
				}
				else
				{
				reqRow.append($('<td><a href="#" class="costCenterBudget_popup_open" onclick="costCenterBudget_overlay(\''+Values[1]+'\','+Values[6]+',\''+Values[2]+'\');">'+Values[5]+"</td>"));
				}					
				reqRow.append($("<td><a href="+urlLink+"> <i class='fa fa-info-circle'></i> </a></td>"));	
        ...
        var row = $("<tr />")
        $("#costCenterResults").append(row);
        row.append($('<td colspan="9"><font style="color: red;font-size: 15px;">No Records to display</font></td>'));
        $(".page_option").css('display', 'none');
    }
    
    $('#costCenterResults').tablesPaginate({navigateType:'navigator'},recordPage,costCenterResults);
   
}

function getSecurityDepositAmountsListLoad(){
	
   
   //alert(CONTENXT_PATH);
   var url = CONTENXT_PATH+'/Requirements/getSecurityDepositAmountsListLoad.action';
    
	var req = initRequest(url);
    req.onreadystatechange = function() {
       document.getElementById('loadingSecurityDepositAmts').style.display = 'block';
        if (req.readyState == 4 && req.status == 200) {
      $('#loadingSecurityDepositAmts').hide();
            // alert("response"+req.responseText);
            populateSecurityDepositAmountsListLoad(req.responseText);
        }
    };
    req.open("GET", url, "true");
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function populateSecurityDepositAmountsListLoad(response) 
{
	// alert(response);
	$(".page_option").css('display', 'block');
var arrayList = response.split("^");
var recordPage=document.getElementById("paginationOption").value;
var table = document.getElementById("securityDepositAmountsResults");
// var name = document.getElementById("account_name").value;
// var table = document.getElementById("contactPageNav");
for (var i = table.rows.length - 1; i > 0; i--) {
	table.deleteRow(i);
...
if (response.length > 0) {
	for (var i = 0; i < arrayList.length - 1; i++) {
		...
		var Values = arrayList[i].split("|");
		{
			var row = $("<tr />")
			$("#securityDepositAmountsResults").append(row);
			
			//row.append($("<td>" + Values[0] + "</td>"));
			row.append($("<td>" + Values[3] + "</td>")); //Cusomer or Vendor Name
			row.append($("<td>" + Values[1] + "</td>"));
			row.append($("<td>" + Values[2] + "</td>"));
			
			row.append($("<td>" + Values[4] + "</td>"));
			row.append($("<td>" + Values[5] + "</td>"));
			// row.append($("<td>" + Values[6] + "</td>")); // Last acitivity By
...
$('#securityDepositAmountsResults').tablePaginate({	navigateType : 'navigator'}, recordPage);
}
