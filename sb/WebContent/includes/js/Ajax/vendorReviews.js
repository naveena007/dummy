...
function addVendorRatingAndReveiw() {
	...
	if (reviewName == null || reviewName == "") {
		$('#reviewNameValidation').html("<font color='red'>You must Enter the Title</font>").show().delay(3000).fadeOut();
	...
	if (comment == "" || comment == null) {
		$('#commentValidation').html("<font color='red'>You must Enter the comments</font>").show().delay(3000).fadeOut();
	...
	req.onreadystatechange = function() {
		if (req.readyState == 4 && req.status == 200) {
			if (req.responseText == "success") {
				...
				$("#successMessage").html("<font color='green'>Comment Added Successfully</font>").show().delay(3000).fadeOut();
				//alert("success");
				...
				$("#successMessage").html("<font color='red'>Failed to Add Comment, Try again...</font>").show().delay(3000).fadeOut();
				//alert("fail")
			}
		}
	};
...
function titleCheck(textBoxId, errorTextId) {
	if ($(textBoxId).val() == '') {
		$(errorTextId).html("<font color='red'>You must enter the title</font>");
...
function commentCheck(textBoxId, errorTextId) {
	if ($(textBoxId).val() == '') {
		$(errorTextId).html("<font color='red'>You must enter the comment</font>");
		$(textBoxId).css('border', '1px solid red');
		setTimeout(function() {
			$(textBoxId).css('border', '');
			$(errorTextId).children().remove();
