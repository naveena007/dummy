			...
			.ajax({
				...
				success : function(data, stat, xhr) {
					...
					if (xhr.getResponseHeader('urlexists') === 'free'
							&& $(textBoxId).val() != '') {
						//$(errorTextId).html("<font color='green'><br>Mail Extension is Available</font>");
			...
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					if (req.responseText == "Not Exist") {
						...
						/*$("#orgExtCheckSpan")
								.html(" <font color='red'><br>Mail Extension Already Exists !</font>");*/
...
function custVenRegValidation() {
	...
	if (orgName == "") {
		...
		$("#orgNameCheckSpan").html("<font color='red'>Please enter name</font>");
	...
	var result=false;
	var captchaValue=document.getElementById("custVenCaptchaText").value;
	//alert(captchaValue);
	result=simpleCaptchaValidation(captchaValue,"CVReg");
	//alert(result);
	if(result===false)
	{
		loadNewCaptcha("CVReg");
		$("#custVenCaptchaText").val("");
	$("#cvRegRecaptchaSpan").html("<font color=red><br>Invalid captcha..</font>");
	setTimeout(function() {
		$("#cvRegRecaptchaSpan").children().remove();
	}, 3000);
	return false;
	}
}
