...
import com.mss.msp.reccruitmentAjax.RecruitmentAjaxHandlerAction;
...
public class DataSourceDataProvider {
	...
	public int getNoOfSubmisions(int req_id, int orgId) throws ServiceLocatorException {
		...
		connection = ConnectionProvider.getInstance().getConnection();
		try {
			queryString = "SELECT COUNT(createdbyOrgId) as count FROM req_con_rel  WHERE  status not like '%SOW%' and csr_status='Approve' and reqId="
					+ req_id;
	 ...
	 * Author:
	 *
	 * ForUse :getSkillsCategory() method is used to get all skills without
	 * Others option used in Questions and Requirement Module
	 *
	 * *****************************************************************************
	 */
	 ...
	 * Author:
	 *
	 * ForUse :getReqSkillsCategory() method is used to get only secondary
	 * skills with Others option
	 *
	 * *****************************************************************************
	 */
	/*
	 ...
	 *
	 * ForUse :getReqPrimarySkillsCategory() method is used to get only primary
	 * skills with Others option
	 *
	 * *****************************************************************************
	 */
	 ...
	 * Author:Tasneem<tshaik@miraclesoft.com>
	 *
	 * ForUse :getAllSkills() method is used to get all skills with others
	 * option used in Consultant module
	 *
	 * *****************************************************************************
	 */

	public Map getAllSkills(int flag) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getSkillsCategory Method Start*********************");
		Connection connection = null;
		Statement statement = null;
	 ...
	 * Author:Tasneem<tshaik@miraclesoft.com>
	 *
	 * ForUse :skillCheck() method is used to
	 *
	 * *****************************************************************************
	 */
	...
	public String getVendorsBasedOnReqCategory(String reqId, int orgId, String reqType, String vendorName,
			int vendorTierType) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getRegionName Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		...
		if ("CO".equals(reqType)) {
			queryString = "SELECT vendor_id,account_name,c.vendor_tier_id FROM customer_ven_rel c "
					+ " LEFT OUTER JOIN accounts a on (account_id=vendor_id)" + " WHERE customer_id= " + orgId
		...
		if (!"".equals(vendorName) && vendorName != null) {
			queryString += " and a.account_name like '%" + vendorName.trim() + "%'";
		}
		if ("CO".equals(reqType)) {
			if (vendorTierType != -1) {
				queryString += " and c.vendor_tier_id = '" + vendorTierType + "'";
			}
		}
		System.out.println("getRegionName :: query string ------>" + queryString);
		try {
			...
			while (resultSet.next()) {
				resultString += resultSet.getInt("vendor_id") + "||" + resultSet.getString("account_name") + "||"
						+ resultSet.getInt("c.vendor_tier_id") + "^^";
			}
		} catch (SQLException ex) {
			ex.printStackTrace();
		} finally {
	...
	/**
	 * *****************************************************************************
	 * Date : July 18, 2017, 11:11 PM IST
	 *
	 * Author: Tasneem<tshaik@miraclesoft.com>
	 *
	 * ForUse : customersNamesForCSR() method is used to getting Customers
	 * related to Csr.
	 *
	 * *****************************************************************************
	 */
	public Map customersNamesForCSR(int csrId) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: customersNamesForCSR Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		Map customerList = new HashMap();
		connection = ConnectionProvider.getInstance().getConnection();

		if (csrId > 0) {

			queryString = "SELECT account_id,account_name FROM accounts acc JOIN csrorgrel csr_org_rel ON (csr_org_rel.org_id=acc.account_id)";

			queryString += " and csr_org_rel.csr_id='" + csrId + "' and csr_org_rel.STATUS='Active'";

			queryString += " ORDER BY account_id DESC ";

		} else {
			queryString = "SELECT account_id,account_name FROM accounts acc";
			queryString += " JOIN acc_address ac_ad ON (acc.account_id=ac_ad.acc_id) JOIN org_rel org ON (org.related_org_Id=ac_ad.acc_id)";
			queryString += " WHERE is_primary=1 AND acc_type='1'";
			queryString += " ORDER BY account_name ASC";
		}

		System.out.println("getCustomerList :: query string ------>" + queryString);
		try {
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				customerList.put(resultSet.getInt("account_id"), resultSet.getString("account_name"));
			}
		} catch (SQLException ex) {
			ex.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: customersNamesForCSR Method End*********************");
		System.out.println("customer Map------------>" + customerList);
		return customerList;
	}
	 ...
	 * Author :
	 *
	 * For Use : getConsult_backgroundAttachmentLocation() is used to get
	 * background attachment location which is uploaded by consultant.
	 *
	 *
	 * *****************************************************************************
	 */
	public String getConsult_backgroundAttachmentLocation(int consult_acc_attachment_id) {
		System.out.println(
				"********************DataSourceDataProvider :: getConsult_backgroundAttachmentLocation Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		String attachmentLocation = "";
		try {
			queryString = "SELECT attachment_path,attachment_name FROM consultantbackgroundcheck WHERE attachment_id="
					+ consult_acc_attachment_id + "";
			System.out.println("getConsult_backgroundAttachmentLocation :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getConsult_backgroundAttachmentLocation Method End*********************");
		return attachmentLocation;
	}
	 ...
	 * Author :
	 *
	 * For Use : getConsult_backgroundStatus() is used to get active attachments
	 * background status.
	 *
	 *
	 * *****************************************************************************
	 */
	public String getConsult_backgroundStatus(int objectId, int reqId) {
		System.out.println(
				"********************DataSourceDataProvider :: getConsult_backgroundStatus Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		...
		try {
			queryString = "SELECT background_status,comments FROM acc_rec_attachment WHERE object_id=" + objectId
					+ " AND req_id=" + reqId + " AND (object_type='EV' OR object_type='CD') AND status='Active'";
			System.out.println("getConsult_backgroundStatus :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				backgroundStatus = resultSet.getString("background_status") + "|" + resultSet.getString("comments");
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
	 ...
	 * Author :
	 *
	 * For Use : getVerificationStatusAndComments() is used to get verification
	 * status and comments by CSR
	 *
	 *
	 * *****************************************************************************
	 */
	public String getVerificationStatusAndComments(int objectId, int reqId) {
		System.out.println(
				"********************DataSourceDataProvider :: getVerificationStatusAndComments Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		String statusAndComments = null;
		try {
			queryString = "SELECT verification_Status,verification_comments FROM consultantbackgroundcheck WHERE object_id="
					+ objectId + " AND req_id=" + reqId + " AND status='Active'";
			System.out.println("getVerificationStatusAndComments :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				statusAndComments = resultSet.getString("verification_Status") + "|"
						+ resultSet.getString("verification_comments");
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getVerificationStatusAndComments Method End*********************");
		return statusAndComments;
	}
	 ...
	 * Author :
	 *
	 * For Use : getCSRListByOrgId-- this method is used to get associated CSR
	 * list with organization
	 *
	 *
	 * *****************************************************************************
	 */
	public Map getCSRListByOrgId(int orgId) {
		System.out.println(
				"********************DataSourceDataProvider :: getCSRListByOrgId Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		...
		try {
			queryString = "SELECT CONCAT (first_name,'.',last_name) as name,csr_id FROM csrorgrel cor LEFT JOIN users u ON cor.csr_id=u.usr_id WHERE cor.org_id='"
					+ orgId + "'  AND cor.STATUS='Active'";
			System.out.println("getCSRListByOrgId :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				csrMap.put(resultSet.getInt("csr_id"), resultSet.getString("name"));
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getCSRListByOrgId Method End*********************");
		return csrMap;
	}
	 ...
	 * Author:
	 *
	 * ForUse :getSecurityDepositFlag() method is used to
	 *
	 * *****************************************************************************
	 */
	public String getSecurityDepositFlag(String orgId) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getSecurityDepositFlag Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		...
		connection = ConnectionProvider.getInstance().getConnection();
		queryString = "SELECT security_deposit FROM acc_basic_info WHERE acc_id=" + orgId;
		System.out.println("getSecurityDepositFlag :: query string ------>" + queryString);
		try {
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				resultString = resultSet.getString("security_deposit");
			}
		} catch (SQLException ex) {
			ex.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getSecurityDepositFlag Method End*********************");
		return resultString;
	}

	public String doUpdateSecurityDepositInTechreview(RecruitmentAction recruitmentaction) {
		System.out.println(
				"********************DSDP :: doUpdateSecurityDepositInTechreview Method Start*********************");
		...
		try {
			...
			callableStatement = connection.prepareCall("{CALL spdoUpdateSecurityDetails(?,?,?,?)}");
			System.out.println(
					"doUpdateSecurityDepositInTechreview :: procedure name : doUpdateSecurityDepositInTechreview ");
			callableStatement.setString(1, recruitmentaction.getConsultId());
			callableStatement.setInt(2, recruitmentaction.getRequirementId());
			callableStatement.setString(3, recruitmentaction.getFinalTechReviewStatus());

			callableStatement.registerOutParameter(4, Types.VARCHAR);
			isExceute = callableStatement.execute();
			resultString = callableStatement.getString(4);
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
			try {
		...
		System.out.println(
				"********************DSDP :: doUpdateSecurityDepositInTechreview Method End*********************");
		return resultString;
	}
	 ...
	 *
	 * ForUse : getCSREmailListByOrgId() method is used to get all CSR email
	 * id's who are associated with given organization.
	 *
	 * *****************************************************************************
	 */
	public List getCSREmailListByOrgId(int orgId) {
		System.out.println(
				"********************DataSourceDataProvider :: getCSREmailListByOrgId Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		ArrayList emailId = new ArrayList();
		try {
			queryString = "SELECT u.email1 as email FROM csrorgrel cor LEFT OUTER JOIN users u ON cor.csr_id=u.usr_id WHERE cor.org_id="
					+ orgId;
			System.out.println("getCSREmailListByOrgId :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
			while (resultSet.next()) {
				emailId.add(resultSet.getString("email"));
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getCSREmailListByOrgId Method End*********************");
		return emailId;
	}
	 ...
	 * Author :
	 *
	 * For Use : updateAcceptanceStatus() method is used to update acceptance
	 * status.
	 *
	 * *****************************************************************************
	 */
	public int updateInterviewNotificationStatus(int ctId, String interviewNotificationStatus, String rejectComments)
			throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: updateAcceptanceStatus Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		int c = 0;
		try {
			connection = ConnectionProvider.getInstance().getConnection();
			String queryString1 = "UPDATE con_techreview SET interviewNotificationStatus='"
					+ interviewNotificationStatus + "',conRejectionComments='" + rejectComments + "'  WHERE id=" + ctId;
			System.out.println("updateCSRNotificationStatus :: query string ------>" + queryString1);
			statement = connection.createStatement();
			c = statement.executeUpdate(queryString1);
		} catch (SQLException ex) {
			ex.printStackTrace();
		...
		System.out.println(
				"********************DataSourceDataProvider :: updateAcceptanceStatus Method End*********************");
		return c;
	}
	 ...
	 * Author :
	 *
	 * ForUse : getCustomerTovendorEmailIds() method is used to getting email
	 * id's of who is forwarding technical review,whom to forward,csr's who are
	 * associated with customer and vendor.
	 *
	 * *****************************************************************************
	 */
	public List getCustomerTovendorEmailIds(RecruitmentAction recruitmentAction) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getCustomerTovendorEmailIds Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String venEmailQueryString = "", forwardToEmailId, forwardByEmailId;
		ArrayList emailId = new ArrayList();
		ArrayList CSREmailId = new ArrayList();

		try {
			venEmailQueryString = "SELECT c.email1 AS venEmail FROM users c "
					+ " LEFT OUTER JOIN req_con_rel rcr ON(rcr.created_By=c.usr_id) " + " WHERE rcr.reqId="
					+ recruitmentAction.getRequirementId() + " AND rcr.consultantId="
					+ recruitmentAction.getConsult_id() + " ";

			System.out.println("getAttachmentDetails :: query string ------>" + venEmailQueryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(venEmailQueryString);
			while (resultSet.next()) {
				emailId.add(resultSet.getString("venEmail"));
			}
			/*
			 * CSREmailId = (ArrayList)
			 * com.mss.msp.util.DataSourceDataProvider.getInstance().
			 * getCSREmailListByOrgId(recruitmentAction.getAccount_id());
			 * ListIterator<String> itr = CSREmailId.listIterator(); while
			 * (itr.hasNext()) { emailId.add(itr.next()); }to forward all csrs
			 * associating with the customer
			 */
			emailId.add(getEmailIdbyuser(recruitmentAction.getModified_by()));
			forwardToEmailId = getCustomerForwardToEmail(recruitmentAction.getContechId());
			forwardByEmailId = getCustomerForwardByEmail(recruitmentAction.getContechId());
			if (forwardToEmailId.equals(forwardByEmailId)) {
				emailId.add(forwardToEmailId);

			} else {
				emailId.add(forwardToEmailId);
				emailId.add(forwardByEmailId);
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getCustomerTovendorEmailIds Method End*********************");
		return emailId;
	}
	 ...
	 * Author :
	 *
	 * ForUse : getCustomerForwardToEmail() method is used to getting email id's
	 * of whom to forward the technical interview
	 *
	 * *****************************************************************************
	 */
	public String getCustomerForwardToEmail(int con_techId) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getCustomerForwardToEmail Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String custForwardToQueryString = "";
		String forwardToEmailId = "";
		try {
			custForwardToQueryString = "SELECT email1 FROM con_techreview ct LEFT OUTER JOIN users u ON ct.forwarded_to=u.usr_id WHERE ct.id="
					+ con_techId;
			System.out.println("getCustomerForwardToEmail :: query string ------>" + custForwardToQueryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(custForwardToQueryString);

			while (resultSet.next()) {
				forwardToEmailId = resultSet.getString("email1");
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
		...
		System.out.println(
				"********************DataSourceDataProvider :: getCustomerForwardToEmail Method End*********************");
		return forwardToEmailId;
	}
	 ...
	 * Author :
	 *
	 * ForUse : getCustomerForwardByEmail() method is used to getting email id's
	 * of whom to forward the technical interview
	 *
	 * *****************************************************************************
	 */
	public String getCustomerForwardByEmail(int con_techId) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getCustomerForwardByEmail Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String custForwardByQueryString = "";
		String forwardByEmailId = "";
		try {
			custForwardByQueryString = "SELECT email1 FROM con_techreview ct LEFT OUTER JOIN users u ON ct.forwarded_by=u.usr_id WHERE ct.id="
					+ con_techId;
			System.out.println("getCustomerForwardByEmail :: query string ------>" + custForwardByQueryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(custForwardByQueryString);

			while (resultSet.next()) {
				forwardByEmailId = resultSet.getString("email1");
			}
		} catch (Exception sqe) {
			sqe.printStackTrace();
		} finally {
	 ...
	 * Author :
	 *
	 * ForUse : getVendorNameByConIdAndReqId() method is used to get vendor name
	 * by using requirement Id and consultant Id
	 *
	 * *****************************************************************************
	 */
	public String getVendorNameByConIdAndReqId(int reqId, int conId) throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: getVendorNameByConIdAndReqId Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		String vendorName = "";
		try {
			queryString = "SELECT a.account_name AS vendorName FROM req_con_rel rcr LEFT OUTER JOIN accounts a ON(rcr.createdbyOrgId=a.account_id) WHERE rcr.reqId="
					+ reqId + " AND rcr.consultantId=" + conId;
			System.out.println("getVendorNameByConIdAndReqId :: query string ------>" + queryString);
			connection = ConnectionProvider.getInstance().getConnection();
			statement = connection.createStatement();
			resultSet = statement.executeQuery(queryString);
		...
		System.out.println(
				"********************DataSourceDataProvider :: getVendorNameByConIdAndReqId Method End*********************");
		return vendorName;
	}
	 ...
	 * Author :
	 *
	 * For Use : updateInterviewNotificationStatusByCSR() method is used to
	 * update acceptance status by CSR
	 *
	 * *****************************************************************************
	 */
	public int updateInterviewNotificationStatusByCSR(int ctId, String interviewNotificationStatus, int csrId)
			throws ServiceLocatorException {
		System.out.println(
				"********************DataSourceDataProvider :: updateInterviewNotificationStatusByCSR Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		int c = 0;
		try {
			connection = ConnectionProvider.getInstance().getConnection();
			String queryString1 = "UPDATE con_techreview SET interviewNotificationStatus='"
					+ interviewNotificationStatus + "',modified_by=" + csrId + "  WHERE id=" + ctId;
			System.out.println("updateCSRNotificationStatus :: query string ------>" + queryString1);
			statement = connection.createStatement();
			c = statement.executeUpdate(queryString1);
		} catch (SQLException ex) {
		...
		System.out.println(
				"********************DataSourceDataProvider :: updateAcceptanceStatusByCSR Method End*********************");
		return c;
	}
	 ...
	 * Author : Tasneem <tshaik@miraclesoft.com>
	 *
	 * ForUse : getRateCardExistOrNot() method is used to get
	 *
	 * *****************************************************************************
	 */
	public boolean getRateCardExistOrNot(String jobtitle)
			throws ServiceLocatorException {
		...
		try {
			System.out.println(
					"********************DSDP :: getRateCardExistOrNot Method Start*********************");
			connection = ConnectionProvider.getInstance().getConnection();
			if (jobtitle != null
					&& !"".equals(jobtitle)) {
				queryString = "select jobtitle from orgratecards where jobtitle='"+jobtitle+"'";
						
				System.out.println("getRateCardExistOrNot ------->" + queryString);
				preparedStatement = connection.prepareStatement(queryString);
				resultSet = preparedStatement.executeQuery();
		...
		System.out.println(
				"********************DSDP :: getRateCardExistOrNot Method End*********************");
		return isRateCard;
	}
	 ...
	 * Author : 
	 *
	 * For Use : getBackground_AttachmentLocation() is used to
	 *
	 *
	 * *****************************************************************************
	 */
	public String getBackground_AttachmentLocation(int consult_acc_attachment_id) {
		System.out.println(
				"********************DataSourceDataProvider :: getBackground_AttachmentLocation Method Start*********************");
		Connection connection = null;
		Statement statement = null;
		ResultSet resultSet = null;
		String queryString = "";
		...
		System.out.println(
				"********************DataSourceDataProvider :: getBackground_AttachmentLocation Method End*********************");
		return attachmentLocation;
	}
