...
function editVendorTierOverlay(tierId,customerId,customerName,vendorName) {
/*alert("in overlay");*/
    $('#tierId').val(tierId);
    ...
    if (response.length > 0) {
        ...
        for (var i = 0; i < techReviewList.length - 1; i++) {
            ...
            var Values = techReviewList[i].split("|");
            {
                ...
                row.append($('<td><a href=../acc/viewAccount.action?accFlag=accDetails&tierFlag=vassociation&accountSearchID='+ Values[8] +">"+ Values[7] +"</a></td>"));
                row.append($('<td><a href="#" class="editVendorTier_popup_open" onclick="editVendorTierOverlay(' + Values[4] + ',' + Values[8] + ',\'' + Values[7] + '\',\'' + Values[5] +'\');">' + Values[5] + "</td>"));
...
function editVendorTierDetails() {
    ...
    //var accountSearchID = $('#accountSearchID').val();
    var accountSearchID = $('#customerId').val();
    ...
    if (flag == true) {
        ...
        req.onreadystatechange = function() {
            if (req.readyState == 4 && req.status == 200) {
                //alert("Success")
            	$("e1").html(
				"<font color=green>Vendor tier updated succesfully</font>");
		setTimeout(function() {
			$("e1").children().remove();
		}, 3000);
                getVendors();
...
function searchVendorTier() {
   ...
   // alert(customerList);
    ...
    var url = 'searchVendorTierDetails.action?vendorTierType=' + vendorTierType + '&TierStatus=' + TierStatus + '&accountSearchID=' + accountSearchID + '&customerList=' + customerList;
    //alert(url)
    var req = initRequest(url);
    req.onreadystatechange = function() {
        document.getElementById('loadingVendor').style.display = 'block';
...
function populateResourceInTable(responce) {
	//alert("response in populate resourceIntable"+responce);
    ...
    var pager = new Pager('resourceTable', 10);
    pager.init();
...
function addCostCenterBudget(budgetStatus) {
    ...
    if (budgetStatus == "SB") {
        ...
        document.getElementById('ccbudget').style.visibility = 'hidden';
        document.getElementById("ccbudget").disabled = true;
    }
...
function costCenter_projects(cccode) {
	...
	//alert(specialBox.style.display);
    if (specialBox.style.display == "block") {
        specialBox.style.display = "none";
    } else {
    	specialBox.style.display = "block";
...
function showProjectsInCostCenter(ccCode) {
	//alert("cccode in show projectsincostcenter"+ccCode);
