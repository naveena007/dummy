			...
			.extend({
				...
				tablesPaginate : function(options, recordPage, tab) {
					...
					if (defaults.table.id == "accountSearchResults") {
						var totalRecords = document
								.getElementById('searchAccountTotalRecords').value;
						// alert("total records are "+totalRecords);
					} else if (defaults.table.id == "reqTableInRecruiter") {
						var totalRecords = document
								.getElementById('totalRecords').value;
					} else if (defaults.table.id == "reqTableInAccount") {
						var totalRecords = document
								.getElementById('reqInAccTotalRecords').value;
					} else if (defaults.table.id == "consultantListTable") {
						//consultantListTable is in submitted profile page.
						var totalRecords = document
								.getElementById('subListTotalRecords').value;
					} else if (defaults.table.id == "costCenterResults") {
						//consultantListTable is in submitted profile page.
						var totalRecords = document
								.getElementById('ccTotalRecords').value;
						//alert("total records are "+totalRecords);
					} else if (defaults.table.id == "techReviewTable") {
						//consultantListTable is in submitted profile page.
						var totalRecords = document
								.getElementById('techReviewTotalRecords').value;
						//alert("total records are "+totalRecords);
					}
					...
					if (defaults.navigateType == 'navigator') {
						...
						nextPage = (nextPage >= totalPages) ? totalPages
								: nextPage;
						prevPage = (prevPage < firstPage) ? firstPage
								: prevPage;

						buildButtons += "<a href='javascript:void(0);' id='"
								+ firstPage
								+ "' class='btn btn-warning pagination-btn'><i style='font-size: 14px; background-color: #396E9D; color: white; border-radius: 13px; width: 165%;'><<</i></a>";
						buildButtons += "<a href='javascript:void(0);' id='"
								+ prevPage
								+ "' class='btn btn-warning pagination-btn'><i style='font-size: 14px; background-color:#56a5ec; color: white; border-radius: 13px; width: 165%;'><</i></a>";
						if (totalPages > 5) {
							if ((totalPages - pno) == 3) {
								buildButtons += "<a href='javascript:void(0);' id='"
										+ (pno - 1)
										+ "' class='btn btn-warning pagination-btn'>"
										+ (pno - 1) + "</a>";
							}
							if ((totalPages - pno) == 2) {
								buildButtons += "<a href='javascript:void(0);' id='"
						...
						for (var i = 1; i <= totalPages; i++) {
							...
							buildButtons += "<button type='button' id='"
									+ i
									+ "' class='btn btn-warning pagination-btn'>"
									+ i + "</button>";
						}
					...
					if (totalRecords > 10) {
						if (defaults.table.id == "reqTableInRecruiter") {
							...
							document
									.getElementById("consultantList_pageNation").innerHTML = buildButtons;
						} else if (defaults.table.id == "costCenterResults") {
							document.getElementById("ccList_pageNation").innerHTML = buildButtons;
						} else if (defaults.table.id == "techReviewTable") {
							document.getElementById("techReview_pagination").innerHTML = buildButtons;
						}
					...
					$(".pagination-btn")
							.on(
									...
									function() {
										var id = $(this).attr("id");
										var pastPage = document
												.getElementById("pgNo").value;
										//  alert(pastPage);
										...
										if (defaults.table.id == "reqTableInRecruiter") {
											...
											getSubmittedListBySearch('0');
										} else if (defaults.table.id == "costCenterResults") {
											getSearchCostCenterList('0');
										} else if (defaults.table.id == "techReviewTable") {
											getSearchTechReviewList('0');
										}
