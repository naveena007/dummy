	...
	req.onreadystatechange = function() {
		if (req.readyState == 4 && req.status == 200) {
			...
			/*}
				...
				permanentSelectedStateSet(req.responseText,stateValue);
			}*/
		}
	};
	req.open("GET", url, "true");
...
function PermanentState(data) {
	// alert(data);
	var stateVal= document.getElementById('consult_State').value;
	var flag = data.split("FLAG");
	var addList = flag[0].split("^");
	...
	if(stateVal!="" && stateVal!=-1 && stateVal!=0){
		//document.getElementById("consult_State").selectedIndex=stateVal;
		document.getElementById("consult_State").value=stateVal;
	}
...
function doSkillValidate(){
	...
	var flagList=primaryList.split("FLAG");
	var totalSkills=flagList[0].split(",");
...
function doSecSkillValidate(){
	...
	//alert(v);
		...
		//var skillValue=  v.replace(/[{()}]/g, '');
		 ...
		 var flagList=secondaryList.split("FLAG");
		var totalSkills=flagList[0].split(",");
      //alert(totalSkills);
...
function populateAttachmentTable(response) {
	...
	for (var i = 0; i < attachmentList.length - 1; i++) {
		...
		var Values = attachmentList[i].split("|");
		{
			...
			if(Values[5]=='In-Active'){
				attach_row.append($("<td> Inactive </td>"));
			}
			else{
...
//alert("file upload");
	...
	if (FileUploadPath == '') {
		...
		if (Extension == "pdf" || Extension == "doc" || Extension == "docx") {
			...
			$("addCnsltError").html("<font color='red'> Please, Upload PDF or Doc or Docx file.</font>");
			//$("#fileSpanId").html("<font color='red'> Allows only doc,docx type or pdf.</font>");
			//document.getElementById("fileSpanId").style.color = red; // red or #ffffff
			return false;
		}
	}
}
...
function conLoginRecaptchaValidation() {
	var response = false;
	var captchaValue = document.getElementById("conLoginRegCaptchaText").value;
	response = simpleCaptchaValidation(captchaValue,"conLoginRegCaptcha");

	if (response === false) {
		
		$("#conLoginRegCaptchaText").val("");
		$("#conLoginRegCaptchaSpan").html(
				"<font color=red><br>Invalid captcha..</font>");
		setTimeout(function() {
			$("#conLoginRegCaptchaSpan").children().remove();
		}, 3000);
		loadNewCaptcha("conLoginRegCaptcha");
	}
...
function consultantValidation() {
	...
	var consultantCaptchaText = $("#consultantCaptchaText").val();
	//alert(consultantCaptchaText);
	/*
...
*/
	...
	var response=false;
	response=simpleCaptchaValidation(consultantCaptchaText,"conReg");
	//alert(response);
	if(response==false)
		{
				loadNewCaptcha("conReg");
				$("#consultantCaptchaText").val("");
		$("#conRegRecaptchaSpan").html(
				"<font color=red><br>Invalid captcha..</font>");
		setTimeout(function() {
			$("#conRegRecaptchaSpan").children().remove();
...
function updateCSRStatus()
{
	  var consult_id = $("#consultidhidden").val();
	    ...
	    if(csrstatus=='-1'){
	        ...
	        $("#resultMessage").show().delay(2000).fadeOut();
	    	return false;
	    }
...
function showBackGroundAttachmentDetails(consult_id,roleId) {
//alert("consult_id-->"+consult_id);
	//alert("role id is "+roleId);
	if(roleId!=15&&roleId!=16)
		{
		var reqId=document.getElementById("conrequirementId").value;
		}
	else
		{
		var reqId=0;
	 ...
	 $.ajax({
      ...
      //   url:'addEventposting.action?eventTitle='+escape(eventtitle)+'&eventType='+eventType+"&eventStatus="+eventStatus+"&startDate="+selectDateFrom+"&endDate="+selectDateTo+"&timeZone="+timeZone+"&eventLocation="+escape(eventLocation)+"&transportation="+escape(transportation)+"&eventDescription="+escape(eventDescription)+"&startTime="+event_time_from+"&endTime="+event_time_to+"&midDayFrom="+midday_from+"&midDayTo="+midday_to+"&eventBoldtitle="+escape(eventBoldtitle)+"&eventRegluarTitle="+escape(eventRegluarTitle)+"&eventRegistrationLink="+escape(eventRegistrationLink)+"&contactUsEmail="+escape(contactUsEmail)+"&conferenceUrl="+escape(conferenceUrl)+"&primaryTrack="+primaryTrack+"&secondaryTrack="+secondaryTrack+"&eventDepartment="+eventDepartment+"&eventAfterVideoUrl="+escape(eventAfterVideoUrl),
		 url:'../consultant/getConsultantBackgroundAttachments.action?consult_id='+ consult_id+'&requirementId='+reqId,
				
          context: document.body,
          success: function(responseText) {
        	  //alert(responseText);
             // displaymessage = responseText;
        	  populateBackgroundAttachmentTable(responseText,roleId);
...
function populateBackgroundAttachmentTable(response,roleId) {
	    ...
	    for (var key in mainJson) {
	    	if (mainJson.hasOwnProperty(key)) {
	           ...
	           //   row.append($('<td><a href=../recruitment/consultant/getConsultantDetails.action?consult_id='+Values[0]+ '&consultFlag='+vendor+ ">" + Values[1] + "</a></td>"));
	    		  if(roleId==15||roleId==16)
            	  {
	    			reqlink="/sb/Requirements/vendorRequirementDetails.action?requirementId="+val["req_id"];
	    			  //Requirements/vendorRequirementDetails.action?requirementId=323
	    					  row.append($("<td><a href="+reqlink+">"+ val["jdid"]+"</a></td>"));
            	  } 
	              ...
	              if(val["con_attachment_id"]!=0)
	              {	  
	            	  row.append($("<td><figcaption><button type='button' id='id' value="
								+ val["con_attachment_id"]
								+ " onclick=doConsultBackgroundAttachmentDownload("
								+ val["con_attachment_id"]
								+ ")><i class='fa fa-download' ></i></button></figcaption></td>"));
	              ...
	              row.append($("<td><figcaption><button type='button' id='id' value="
	            		  ...
	            		  {
	            	  row.append($("<td><figcaption><button type='button' id='id' disabled><i class='fa fa-download' ></i></button></figcaption></td>"));
	            		  }
	            	  }
	              row.append($("<td>" + val["con_opp_created_date"] + "</td>"));
	              
	                if(val["backgroundStatus"]=="Submitted"||val["backgroundStatus"]=="Initiated")
	            		  {
	                	if((roleId!=15&&roleId!=16&&roleId!=1&&roleId!=11)&&val["status"]=="Active")
	                		{
	              row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="updateBackgroundStatus('+ val["object_id"]+','+val["req_id"]+','+val["acc_attachment_id"]+',\''+val["backgroundStatus"]+'\',\''+val["vStatus"]+'\',\''+val["vComments"]+'\','+roleId+');" data-toggle="modal" data-target="#backgroundStatusDiv">'+ val["backgroundStatus"] +'</td>'));
	                		}
	                	else
	                		{
	                		row.append($("<td>" + val["backgroundStatus"] + "</td>"));	
	                		
	                		}
	            		  }
	            	  else if(val["backgroundStatus"]=="Verifying")
	            		  {
	            		  if(roleId==1)
	            			  {
	            		  row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="updateBgStatusByCSR('+ val["object_id"]+','+val["req_id"]+','+val["acc_attachment_id"]+');" data-toggle="modal" data-target="#bgStatusbyCSRDiv">'+ val["backgroundStatus"] +'</td>'));
	            			  }
	            		  else if(roleId==15||roleId==16)
	            			  {
	            	  ...
	            	  else if(val["backgroundStatus"]=="Verified")
            		  {
	            		  if(roleId==1&&val["status"]=="Active")
	            			  {
	            			  row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="viewVerificatonStatus(\''+val["vStatus"]+'\',\''+val["vComments"]+'\',\''+val["cvComments"]+'\',\''+val["backgroundStatus"]+'\');" data-toggle="modal" data-target="#viewVerificationStatusDiv">'+ val["backgroundStatus"] +'</td>'));
	            		    }
	            		  else if(roleId==15||roleId==16)
	            			  {
	            			  row.append($("<td>Submitted</td>"));
	            			  }
	            		  else if((roleId==13||roleId==3||roleId==11)&&(val["status"]=="Active"))
	            			  {
	            			  row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="updateBackgroundStatus('+ val["object_id"]+','+val["req_id"]+','+val["acc_attachment_id"]+',\''+val["backgroundStatus"]+'\',\''+val["vStatus"]+'\',\''+val["vComments"]+'\','+roleId+');" data-toggle="modal" data-target="#backgroundStatusDiv">'+ val["backgroundStatus"] +'</td>'));
	            			  }
	            		  else
	            	  ...
	            	  else if(val["backgroundStatus"]=="Rejected")
            		  {
	            		  row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="viewVerificatonStatus(\''+val["vStatus"]+'\',\''+val["vComments"]+'\',\''+val["cvComments"]+'\',\''+val["backgroundStatus"]+'\');" data-toggle="modal" data-target="#viewVerificationStatusDiv">'+ val["backgroundStatus"] +'</td>'));
            		  }
	            	  else
...
function uplaodBackgroundScannedCopyUpload(consultantId,attachmentId,requirementId){
	//alert("consultantId-->"+consultantId);
	...
	$("#current_attachmentId").val(attachmentId);
	$("#backgroundAttachmentUploadBtn").click();
	
}

function doConsultBackgroundAttachmentDownload(acc_attachment_id) {

	$("#resume").html("");
	var consult_id = $("#consult_id").val();
	var consultFlag = $("#consultFlag").val();
	var consultantFlag = $("#consultantFlag").val();
	var consultantBackgroundFlag="yes";//this flag is used to download background attachments which are uploaded by consultant. 
	//alert(consultFlag);
	$.ajax({
        url:CONTENXT_PATH + "/recruitment/consultant/checkBgAttachmentExistOrNot.action?acc_attachment_id="+acc_attachment_id,//
        context: document.body,
        success: function(responseText) {
        	//alert(responseText)
        	if(responseText=='Existed'){
...
function updateBackgroundStatus(consultantId,reqId,attachmentId,status,vStatus,vComments,roleId){
...
function updateBgStatusByCSR(consultantId,reqId,attachmentId){
	//alert("hello");
	//alert(consultantId +"--"+reqId+"--"+attachmentId);
...
function viewVerificatonStatus(vStatus,vComments){
	//alert(consultantId +"--"+reqId+"--"+attachmentId);
