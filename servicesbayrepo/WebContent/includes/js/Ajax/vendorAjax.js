    ...
    req.onreadystatechange = function() {
        if (req.readyState == 4) {
            ...
            if (req.status == 200) {
                ...
                $("#resultMessage").html("<font color='green'>Association Updated Successfully</font><br>").show().delay(3000).fadeOut();
            }
        }
        else {
        }
    ...
    if (validateVendorAssociation())
    {
        ...
        req.onreadystatechange = function() {
            if (req.readyState == 4 && req.status == 200) {
                ...
                // $("saveVendorAssociation").html(" <b><font color='red'>
				// Requirement already exists</font></b>");
                getVendorAssociationDetails()
            }
        };
        req.open("GET", url, "true");
...
function validateVendorAssociation() {
    ...
    if (vendorNames == null) {
        // alert("desc");
        $("saveVendorAssociation").html("<font color='red'>Please select Vendor Names</font>");
...
function populateReqTable(response) {
    ...
    for (var i = 0; i < reqList.length - 1; i++) {
        ...
        var Values = reqList[i].split("|");
        {
            ...
            // reqRow.append($('<td><a
			// href="../Requirements/requirementedit.action?account_name='+name+'&accountSearchID='+accountSearchID+'&accountFlag=Account&requirementId='+Values[0]+'"
			// > ' + Values[1] + "</td>"));
    ...
    if (response.length > 0) {
        for (var i = 0; i < eduList.length - 1; i++) {
            ...
            var Values = eduList[i].split("|");
            {
                ...
                $("#vendorAssociationResults").append(row); // this will append
															// tr element to
															// table... keep its
															// reference for a
															// while since we
				...
				// class="vendorAssoEdit_popup_open"
				// onclick="associationEditOverlay('+Values[0]+');" > ' +
				// Values[1] + "</td>"));
                row.append($("<td>" + Values[1] + "</td>"));
                if (Values[7] > 0) {
...
function getConsultantListBySearch() {
    ...
    var url = 'Requirements/getConsultantListBySearch?RequirementId=' + RequirementId + '&consult_name=' + consult_name +
            '&consult_email=' + consult_email + '&consult_skill=' + consult_skill + '&consult_phno=' + consult_phno + '&vendor=' + vendorName + '&consult_lstname=' + consult_lstname + '&consult_ssnNo=' + consult_ssnNo +'&yearExperience='+yearExperience+'&csrStatus='+csrStatus;
    var req = initRequest(url);
    req.onreadystatechange = function() {
...
function consultantListGridDisplay(response) { 
    ...
    var res;
// if(vendor!='yes'){
// var res="Vendor" + "|" + "Candidate Name" + "|" + "Submitted Date" +"|" +
// "SSN No" + "|" + "Skills" +"|"+ "Status" + "|" + "Rate" + "^";
// }
// else
// {
// var res="Candidate Name" + "|" + "Submitted Date"+"|" + "SSN No" + "|" +
// "Email" + "|" + "Skills" +"|" + "Phone Number"+ "|" + "Status" + "|"+ "Rate"
// + "^";
// }
    if (response.length > 0) {
        for (var i = 0; i < eduList.length - 1; i++) {
...
// if(vendor!='yes'){
                res = Values[16];
                // alert("query string is "+res);
// res+(Values[7] +"|"+ Values[1]+"|"+ Values[11]+"|"+Values[14]+"|"+
// Values[3]+"|"+Values[5]+"|$"+ Values[8]+"/Hr^");
// }
// else
// {
// res=res+(Values[1]+"|"+ Values[11]+"|"+ Values[14]+"|"+ Values[2]+"|"+
// Values[3]+"|"+ Values[4]+"|"+ Values[5]+"|$"+ Values[8]+"/Hr^");
// }
//                
                ...
                // row.append($('<td><a
				// href=../recruitment/consultant/editConsultantDetails.action?vendor='+
				// vendor +'&consult_id='+Values[0]+'&consultFlag='+consultFlag
				// +'&requirementId='+requirementId+'&accountSearchID='+accountSearchID+'&accountFlag='+accountFlag+'&jdId='+jdId+'&customerFlag='+customerFlag+">"+
				// Values[1] +"</a></td>"));
                ...
                if (accountFlag != 'csr')
                {
                    ...
                    else
                    {
                        if (vendor == 'yes') {
                            ...
                            // row.append($('<td><a href="#"
							// class="conWithdraw_popup_open"
							// onclick="consultantWithdrawnPopup(\'' + okDisable
							// + '\',\'' + Values[13] + '\');">' + Values[5] +
							// '</td>'));
                            if (Values[5] == 'Processing')
                            {
                            row.append($("<td>" + Values[5] + "</td>")); // need
																			// to
																			// put
																			// overlay
																			// for
              ...
              if(roleId == 1 || roleId == 11){
                ...
                // {
                // row.append($('<td >' + "<img
				// src='./../includes/images/user-login.png' height='20'
				// width='30' style='opacity: 0.3' > " + "</td>"));
                // }
                // if(accountFlag=='csr'&& Values[5]=='Selected' )
                // {
                // row.append($('<td ><a href="#"
				// class="consultantLoginOverlay_popup_open"
				// onclick="consultantOverlayLogin(\'' + Values[2] + '\',\'' +
				// Values[0] + '\',\'' + Values[5] + '\')">' + "<img
				// src='./../includes/images/user-login.png' height='20'
				// width='30' > " + "</td>"));
                // }
                ...
                // {
                // row.append($('<td><center><a href=#
				// class="Migration_popup_open" onclick="migration_overlay(\'' +
				// Values[2] + '\',\'' + Values[0] + '\');">'+'<img
				// src="../includes/images/userMigration.png" height="25"
				// width="25"></center></td>'));
                ...
                // else
                // { // row.append($('<td><center><a href="#"
				// onclick="doMigration('+Values[0]+');">'+"<img
				// src='./../includes/images/migration.png' height='25'
				// width='25' ></center></td>"));
                //                     
                // if(RequirementTaxTerm=='PE')
                // {
                ...
                // }
                // //
				// row.append($("<td><figcaption>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button
				// type='button' value=click onclick=doMigration("+ Values[0]
				// +")><img src='./../includes/images/migration.png' height='20'
				// width='20' ></button></figcaption></td>"));
                // else
                // row.append($("<td><center> -</center> </td>"));
                // }
                //                     
...
// //alert(techSearch);
// window.location =
// '../recruitment/consultant/consultDownloadAttachment.action?acc_attachment_id='
// + acc_attachment_id + '&requirementId=' + reqId + '&accountSearchID=' +
// accountId +
// '&accountFlag=' + accFlag + '&customerFlag=' + customerFlag +
// '&consult_name=' + consult_name + '&consult_email=' + consult_email +
// '&consult_skill=' + consult_skill + '&consult_phno=' + consult_phno +
// '&techSearch=' + techSearch + '&jdId=' + jdId + '&vendor=' + vendor;
}
    ...
    if (response.length > 0) {
        for (var i = 0; i < reqList.length - 1; i++) {
            ...
            var Values = reqList[i].split("|");
            {
                ...
                var reqRow = $("<tr />")
                $("#reqTableInAccount").append(reqRow); // this will append tr
														// element to table...
														// keep its reference
														// for a while since we
														// will add cels into it
                ...
                /*
				 * if(Values[11]=='null'||Values[11]==""){ Values[11]="";
				 * reqRow.append($('<td><a href="#"
				 * class="preSkillOverlay_popup_open" onclick="
				 * showPreferedSkillDetailsCSR('+Values[0]+');"
				 * >'+Values[11].substr(0,10)+"</td>")); }else{
				 * reqRow.append($('<td><a href="#"
				 * class="preSkillOverlay_popup_open" onclick="
				 * showPreferedSkillDetailsCSR('+Values[0]+');"
				 ...
				 */
                reqRow.append($("<td>" + Values[4] + "</td>"));
                reqRow.append($("<td>" + Values[17] + "</td>")); // no.of
																	// submissions
                if(Values[4]=='Opened'){
 ...
 * window.location=
 * "../acc/getReleaseVendors.action?requirementId="+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm; }
 * 
 * 
 * 
 * function getReleaseVendorsListForCsr(){
 * document.getElementById("vendorReleaseButton").style.display = "block"; //
 * document.getElementById('releaseVendorDiv').innerHTML = ''; var
 * requirementId= document.getElementById("requirementId").value; var orgId=
 * document.getElementById("orgId").value; var taxTerm=
 * document.getElementById("taxTerm").value; var vendorName=
 * document.getElementById("vendorName").value; var tier=
 * document.getElementById("tier").value;
 * 
 * var
 * url='./getVendorsBasedOnReqCategoryForCSR.action?requirementId='+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm+'&vendor='+vendorName+'&vendorTierType='+tier;
 * var req=initRequest(url); req.onreadystatechange = function() { if
 ...
 *  } } }; req.open("GET",url,"true");
 * req.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
 * req.send(null); return false; } function
 * populateReleaseVendorsListForCSR(data){ var vendorsIdString=''; var
 * flag=data.split("FLAG"); var addList=flag[0].split("^^"); var table =
 * document.getElementById("releaseReqTable"); for (var i = table.rows.length -
 * 1; i > 0; i--) { table.deleteRow(i); } var row = $("<tr/>")
 * $("#releaseReqTable").append(row); //this will append tr element to table...
 * keep its reference for a while since we will add cels into it if (flag.length >
 * 0) {
 * 
 * for (var i = 0; i < addList.length-1; i++) { var Values =
 * addList[i].split("||"); // var option = document.createElement('li');
 * 
 * var row; var cell; row = document.createElement("TR");
 * 
 * 
 * cell = document.createElement("TD");
 * 
 * cell = document.createElement("TD");
 * cell.appendChild(document.createTextNode(Values[1])); row.appendChild(cell);
 * 
 * cell = document.createElement("TD"); if(Values[2]==1){ cell.innerHTML =
 * "Tier-1";
 * 
 * }else if(Values[2]==2){ cell.innerHTML = "Tier-2";
 * 
 * }else if(Values[2]==3){ cell.innerHTML = "Tier-3";
 *  } row.appendChild(cell);
 * 
 * cell = document.createElement("TD"); if(Values[3]==1){
 * 
 * cell.innerHTML ="Yes";
 * 
 * 
 * }else{ cell.innerHTML ="No"; } row.appendChild(cell);
 * 
 * 
 * cell = document.createElement("TD"); var checkbox =
 * document.createElement('input'); checkbox.type = "checkbox"; checkbox.name =
 * "vendor"+Values[0]; checkbox.value = 0; checkbox.id = "vendor"+Values[0];
 ...
 * 
 * $("#releaseReqTable").append(row); //this will append tr element to table...
 * keep its reference for a while since we will add cels into it
 *  // row.append($("<td>" + Values[1] + "</td>"));///alert("oooooooooooooooo"+$(label)) } }
 * else { var row = $("<tr />") $("#releaseReqTable").append(row);
 * row.append($('<td colspan="5"><font style="color: red;font-size: 15px;">No
 * Records to display</font></td>')) }
 * document.getElementById("vendorsIdString").value = vendorsIdString;
 * $('#releaseReqTable').tablePaginate({ navigateType: 'navigator' },
 * recordPage); pag.init(); pag.showPageNav('pager', 'pageNavPosition');
 * pag.showPage(1); }
 * 
 * 
 * 
 * function onClickHandlerForCSR(res){ alert("res..."+res); var check
 * =res.checked; // alert(check) if (check ==true){ var vendorsIdString =
 * document.getElementById("vendorsIdString").value;
 * alert("vendorsIdString.."+vendorsIdString) var
 * vendorIds=vendorsIdString.split("^^"); var vendorOrgIds=''; for(var i =0;i<vendorIds.length-1;i++){
 * document.getElementById("vendor"+vendorIds[i]).checked=false;
 * $("#vendor"+vendorIds[i]).attr("disabled", true); } } else{ var
 * vendorsIdString = document.getElementById("vendorsIdString").value; var
 * vendorIds=vendorsIdString.split("^^"); var vendorOrgIds=''; for(var i =0;i<vendorIds.length-1;i++){
 * $("#vendor"+vendorIds[i]).attr("disabled", false); } } } function
 * doReleaseRequirementForCSR(){ var count =0; var vendorsIdString =
 * document.getElementById("vendorsIdString").value; var id
 * =document.getElementById("requirementId").value ; var orgId=
 * document.getElementById("orgId").value ; var
 * taxTerm=document.getElementById("taxTerm").value ; var
 * vendorIds=vendorsIdString.split("^^"); var vendorOrgIds=''; var
 * check1=document.getElementById("allVendors").checked; if(check1 ==true){
 * vendorOrgIds = "All"; count = 1; } for(var i =0;i<vendorIds.length-1;i++){
 * var check=document.getElementById("vendor"+vendorIds[i]).checked;
 * if(check==true){ if (count>=1){ vendorOrgIds += ","+vendorIds[i]; }else{
 * vendorOrgIds += vendorIds[i]; } count = count +1; } }
 * alert("vendorOrgIds..."+vendorOrgIds); if(count==0){
 * $("releaseMessage").html(" <font color='red'>Please select atleast one vendor</font>");
 * $("#reqResultMsg").fadeOut(5000); return false; }
 * $("releaseMessage").html(""); $("reqResultMsg").show(); var requirementId=id;
 * var url='./doReleaseRequirementsForCSR.action?requirementId='+requirementId+
 * '&orgId='+orgId+'&taxTerm='+taxTerm+'&vendorOrgIds='+vendorOrgIds;//alert(url)
 * var req=initRequest(url); req.onreadystatechange = function() { if
 * (req.readyState == 4 && req.status == 200) { if(req.responseText==0){
 * $("releaseMessage").html(" <font color='red'>Requirement already Released</font>"); }
 * else{ if(vendorOrgIds=="All"){ $("releaseMessage").html(" <font
 * color='green'>Requirement release for all the Associated Vendors.</font>"); }
 * 
 * else if(taxTerm=='CO'){ $("releaseMessage").html(" <font
 * color='green'>Requirement release for Tier1 Vendors</font>"); }else{
 * $("releaseMessage").html(" <font color='green'>Requirement release for Head
 * Hunters Vendors</font>"); } }
 * document.getElementById("vendorReleaseButton").style.display = 'none'; //
 * getSearchRequirementsList(); $("#reqResultMsg").fadeOut(5000); } };
 * req.open("GET",url,"true");
 * req.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
...
function getVendorNames() {
    ...
    // var accountSearchID = $('#accountSearchID').val();
    var accountSearchID = $('#customerList').val();
    if(accountSearchID==""){
        $("#customerValidation").html(" <font color='red'>Customer field is required</font>").show().delay(3000).fadeOut();;
        return false;
    }
...
function saveVendorTierDetails() {
    ...
    // alert("new customer List"+customerList);
    // var vendorStatus=$('#vendorStatus').val();
    ...
    if (customerList == "") {
        flag = false;
        $("e").html("<font color='red'>Select Customer</font>").show().delay(5000).fadeOut();
    }
    ...
    if (flag == true) {
        var url = CONTENXT_PATH + '/acc/saveVendorTierDetails.action?vendorId=' + vendorId + '&VendorTierId=' + vendorTier + '&accountSearchID=' + customerList + '&pf=' + pf + '&vendorComments=' + vendorComments;
        // alert(url);
        var req = initRequest(url);
        req.onreadystatechange = function() {
            if (req.readyState == 4) {
                if (req.status == 200) {
                    ...
                    if (req.responseText == 'Success') {
                        $("e").show();
                        $("e").html(
        				"<font color='green'>Vendor added succesfully</font>");
        		setTimeout(function() {
        			$("e").children().remove();
        		}, 3000);
        		 		$('#customerList').val('');
...
function getOrgConsultant(vendorName, ven_id)
{
   ...
   // document.getElementById("headingmessage").innerHTML = 'Submitted List<i  class="fa fa-angle-up" id="updownArrowAccount" onclick="toggleContentAccount(\'consultantSearchForm\')" style="margin-top: 0vw;position:absolute;color:#56a5ec"> </i>';
    ...
    // alert(RequirementId);
    // //var
	// url='Requirements/getConsultantList.action?RequirementId='+RequirementId;
    //    
    // var
	// url='Requirements/getConsultantListBySearch?RequirementId='+RequirementId+'&vendor='+vendorName;
    // alert(url)
    var req = initRequest(url);
    req.onreadystatechange = function() {
        document.getElementById('loadingConsultant').style.display = 'block';
...
function populateVendorDashboard(response) {
    ...
    if (response.length > 0) {
        ...
        for (var i = 0; i < reqList.length - 1; i++) {
            ...
            {
                var reqRow = $("<tr />")
                $("#dashboardResults").append(reqRow); // this will append tr
														// element to table...
														// keep its reference
														// for a while since we
														// will add cels into it
                // reqRow.append($("<td>" + Values[0] + "</td>")); For req.Won
                // reqRow.append($("<td>" + Values[1] + "</td>")); For
				// req.Lost..As of now These two not required .
                reqRow.append($("<td>" + Values[0] + "</td>")); // For Month
                reqRow.append($("<td>" + Values[1] + "</td>")); // Requirement
																// count
...
function saveSOWFinderFeeType() {
    ...
    // alert("HI "+SOWSelectValue+" "+RequirementTaxTerm+" "+rateSalary+"
	// "+vendorId+" "+conId+" "+reqId)
...
function logoUpload() {
    ...
    if (filePath != "") {
        $.ajaxFileUpload({
            ...
            fileElementId: 'file', // id <input type="file" id="logo"
									// name="logo" />
            dataType: 'json', // json
    ...
    if (filePath != "")
    {
        ...
        if (Extension == "gif" || Extension == "png" || Extension == "bmp"
            ...
            if (sizeInMb > sizeLimit) {
                ...
                // $("#formValidationMsg").html("<font color='red'>File size
				// must be less than 2MB.</font>");
                return false
            }
        }
        else {
            ...
            // $("#formValidationMsg").html("<font color='red'>please upload
			// file types of GIF, PNG, JPG, JPEG and BMP.</font>");
            return false;
        }
    }
}
function getSearchAccList(element)
{
   ...
   /*
	 * var staffingModearray = []; $("#staffingModeList
	 * :selected").each(function() { staffingModearray.push($(this).text()); });
	 */
    ...
    if ((document.getElementById("searchAccountTotalRecords").value) > 0) {
        for (var i = 0; i < accList.length - 1; i++) {
            ...
            var Values = accList[i].split("|");
            {
                ...
                var reqRow = $("<tr />")
                $("#accountSearchResults").append(reqRow); // this will append
															// tr element to
															// table... keep its
															// reference for a
															// while since we
                    ...
                    // reqRow.append($('<td><a
					// href="../acc/viewAccount.action?accountSearchID=' +
					// Values[0] + '&accountFlag='+accFlag
					// +">"+Values[1]+"</a></td>"));
                    // reqRow.append($("<td><a href='" + Values[1]
					// +"'></a></td>"));
