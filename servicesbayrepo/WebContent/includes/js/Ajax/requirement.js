        ...
        for(var i=0;i<reqList.length-1;i++){     
            ...
            {  
                ...
                if(vendor!='yes'){
                    ...
                    {
                        if(role==3 || role==13){
                        	// reqRow.append($('<td><center><a href="#"
							// class="releaseVendorOverlay_popup_open"
							// onclick="SendReqNotificationToCSR('+Values[0]+','+Values[20]+')">'+"<i
							// class='fa fa-arrow-circle-o-right
							// fa-size'></i></center></td>"));
                        	reqRow.append($('<td><center><a href="#" class="addAuthAccOverlay_popup_open" onclick="popupForSecurityDepoInReqList('+Values[0]+','+Values[20]+')">'+"<i class='fa fa-arrow-circle-o-right fa-size'></i></center></td>"));
                        }
                        else
                        {
                    	...
                    	if(role==1)
                    		{
                    	reqRow.append($('<td><center><a href="#" class="releaseVendorOverlay_popup_open" onclick="getReleaseVendorsForCsr('+Values[0]+','+Values[20]+',\''+Values[14]+ '\');">'+"<i class='fa fa-arrow-circle-o-right fa-size'></i></center></td>"));
                    		}
                    	else
                        {
                            reqRow.append($('<td><center><a href="#" >'+"<i class='fa fa-arrow-circle-o-right fa-size' style='opacity: 0.3' ></i></center></td>"));       
                ...
                if(vendor=='yes'){
                    if(Values[4]=='Closed'||Values[4]=='Released'){
                        ...
                        reqRow.append($('<td><center><a href="../../recruitment/consultant/doAddConsultantForReq.action?requirementId='+Values[0]+'&jdId='+Values[12]+'&jobTitle='+Values[1]+'&targetRate='+Values[15]+'&maxRate='+Values[16]+'&orgid='+orgid+'&reqSkill='+Values[10]+'&vendor='+vendor+'">'+"<i class='fa fa-user-plus fa-size'></i></center></td>"));      
                    }
                }
                if(role==3){
...
function popupForSecurityDepoInReqList(reqId,orgId) { 
	
	// alert(reqId+"--->"+orgId);
	
	securityDepoReqOverlay();
	
	document.getElementById("overlayReqId").value=reqId;
	document.getElementById("overlayOrgId").value=orgId;
	
	 document.getElementById("releaseBtn").disabled = false;
	
	// getSecurityDepoForReq(reqId,orgId);
	getAllSecurityDepositList();
	
}

function getSecurityDepoForReq(){
	
	var securityDepositId=document.getElementById("overlaySecurityDepositList").value;
	
	 var url=CONTENXT_PATH + '/recruitment/consultant/getSecurityDepoForReq.action?securityDepositId='+securityDepositId;
	 // alert("getSecurityDepoForReq-->"+url);
	   var req=initRequest(url);
	   req.onreadystatechange = function() {
		 // document.getElementById('loadingVendorSearch').style.display =
			// 'block';
	       if (req.readyState == 4 && req.status == 200) {
	    	 // $('#loadingVendorSearch').hide();
	         // alert(req.responseText);
// if((req.responseText)==""){
// document.getElementById("securityDepositViewBtn").style.display="none";
// }
// else{
	        	 var Values = (req.responseText).split("|");
		       
	  	       	 document.getElementById("overlayProfileSubmissionCost").value=Values[1];
	  	    	 document.getElementById("overlayProfileRejectionCost").value=Values[2];
	  	    	 document.getElementById("overlayFakeCandidateFine").value=Values[3];
	  	    	 document.getElementById("overlayComments").value=Values[4];
	  	    	document.getElementById("overlayProfileSelectedCost").value=Values[5];
	  	    	// document.getElementById("securityDepositViewBtn").style.display="block";
...
function getAllSecurityDepositList(){
	// var id = document.getElementById('overlaySecurityDepositList').value;
	    // alert(id);
	    var url = CONTENXT_PATH +'/recruitment/consultant/getAllSecurityDepositList.action';
	    ...
	    req.onreadystatechange = function() {
	        if (req.readyState == 4 && req.status == 200) {
	            ...
	            setSecurityDepositList(req.responseText);
	        }
	    };
	    req.open("GET", url, "true");
	    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function setSecurityDepositList(data) {
    // alert(data);
    var securityDepositList = document.getElementById("overlaySecurityDepositList");
    var flag = data.split("FLAG");
    var addList = flag[0].split("^");
    var $select = $('#overlaySecurityDepositList');
    $select.find('option').remove();
    $('<option>').val(-1).text('Please Select').appendTo($select);
...
function securityDepoReqOverlayClose(){
	
	
	var specialBox = document
			.getElementById('addAuthAccBox');
	if (specialBox.style.display == "block") {
		specialBox.style.display = "none";
	} else {
		specialBox.style.display = "block";
...
function securityDepoReqOverlay(){
...
// row.append($("<th>").append("HeadHunters Vendors List"));
        ...
        for (var i = 0; i < addList.length-1; i++) {
          ...
          var  checkbox = document.createElement('input');
           ...
           var label = document.createElement('label')
            ...
            row.append($("<td>").append(checkbox).append(label));
    ...
    var pag = new Pager('releaseReqTable', 5);
    pag.init();
...
function getConsultantListBySearch(){
    ...
    var accountFlag=$('#accountFlag').val();
    var url='Requirements/getConsultantListBySearch?RequirementId='+RequirementId+'&consult_name='+consult_name+'&consult_lstname='+consult_lstname+
    '&consult_email='+consult_email+'&consult_skill='+consult_skill+'&consult_phno='+consult_phno+'&yearExperience='+yearExperience+'&consult_ssnNo=' + consult_ssnNo ;
    var req=initRequest(url);
    req.onreadystatechange = function() {
        document.getElementById('loadingsubmitted').style.display = 'block';
        if (req.readyState == 4 && req.status == 200) {
            $('#loadingsubmitted').hide();
...
function getSubmittedListBySearch(element){
	var pgNo=document.getElementById("pgNo").value;
	//alert("PGNO-->"+pgNo);
    var pGflag=element;
    if(pGflag=='1'){
    ...
    '&consult_email='+consult_email+'&submittedFrom='+submittedFrom+'&submittedEnd='+submittedEnd+'&consultStatus='+consultStatus+'&csrStatus='+csrStatus+'&pgFlag='+pGflag+'&pgNo='+pgNo+'&recordPage='+recordPage;
    var req=initRequest(url);
    req.onreadystatechange = function() {
        ...
        if (req.readyState == 4 && req.status == 200) {
        	...
        	submittedListGridDisplay(req.responseText,element);
        } 
    };
    req.open("GET",url,"true");
    req.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
...
function submittedListGridDisplay(response,flag) {
	//alert(response);
	var dataArray = response.split("###");
    ...
    var vendor = $('#vendor').val();
    // alert("vendor value "+vendor);
    var accountSearchID = $('#accountSearchID').val();
    //alert(accountSearchID);
    ...
    if(flag=='1'){
          //alert("total records are "+dataArray[1]);
            document.getElementById("pgNo").value=1;
            document.getElementById("subListTotalRecords").value=dataArray[1];
        }
      
        var eduList = dataArray[0].split("^");
    ...
    var res;
    //alert(document.getElementById("subListTotalRecords").value);
 if ((document.getElementById("subListTotalRecords").value) > 0) {
        for (var i = 0; i < eduList.length - 1; i++) {
            ...
            var Values = eduList[i].split("|");
            {
             ...
             /*   $("#action_description").tooltip('hide')
              ...
              //  row.append($("<td>" + Values[27] + "</td>")); // Requirement name
              //  alert(Values[27]);
                ...
                var v_comments = Values[15].trim().replace(/ /g, '%20');
                  ...
                  // row.append($('<td><a
                   ...
                   row.append($('<td><a href=../recruitment/consultantLogin/getConsultantProfile.action?pageFlag='+pageFlag+'&consult_id=' + Values[0] + '&requirementId=' + Values[21] + '&accountFlag='+Values[22]+'&consultStatus='+Values[5]+ '&vendor='+vendor+'&accountSearchID=' + orgid + ">"+Values[1]+"</a></td>"));
               ...
               /* row.append("<td>" + Values[17] + "</td>"); */
                ...
                if(Values[23]=="null"||Values[23]=="")
                {
                	if(Values[22]!=1)
                		{
                		row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="uploadBackgroundAttachment('+ Values[0]+','+Values[21]+','+orgid+',\''+pageFlag+'\',\''+vendor+'\');">Click to Initiate</td>'));
                		}
                	else
                		{
                		row.append($('<td>Need to Initiate</td>'));
                		}
                		
                }
                else if(Values[23]=="Submitted"||Values[23]=="Initiated")
                {
                	if(Values[22]==1||Values[22]==11||Values[22]==15||Values[22]==16)
                	{
                		row.append($('<td>' + Values[23] + '</td>'));
                	}
                	else 
            		{
                		row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="updateBackgroundStatus('+ Values[0]+','+Values[21]+','+Values[6]+',\''+Values[23]+'\',\''+Values[25]+'\',\''+Values[26]+'\','+Values[22]+','+orgid+',\''+pageFlag+'\',\''+vendor+'\');" data-toggle="modal" data-target="#backgroundStatusDiv">'+Values[23]+'</td>'));            		}
                	}
                else if(Values[23]=="Verifying")
                {
                	if(Values[22]==1)
            		{
                		row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="updateBgStatusByCSR('+ Values[0]+','+Values[21]+','+Values[6]+','+orgid+',\''+pageFlag+'\',\''+vendor+'\');" data-toggle="modal" data-target="#bgStatusbyCSRDiv">'+Values[23]+'</td>'));
            		}
                	else if(Values[22]==15||Values[22]==16)
                		{
                		row.append($('<td>Submitted</td>'));
                		}
                	else
                		{
                		row.append($('<td>' + Values[23] + '</td>'));
                ...
                else if(Values[23]=="Verified")
                {
                	
                if(Values[22]==1)
                	{
                	row.append($('<td><a href="#" class="conWithdraw_popup_open" onclick="viewVerificatonStatus(\''+Values[25]+'\',\''+Values[26]+'\',\''+Values[24]+'\',\''+Values[23]+'\');" data-toggle="modal" data-target="#viewVerificationStatusDiv">'+Values[23]+'</td>'));
                ...
                row.append($("<td><figcaption><button type='button' value=" + Values[6] + " onclick=doSubmittedAttachmentDownload(" + Values[6] + ")><i class='fa fa-download ' ></i></button></figcaption></td>"));
               
                // if (vendor != 'yes' && accountFlag != 'csr') {
                    row.append($('<td><a href=/sb/Requirements/techReview.action?pageFlag='+pageFlag+'&requirementId=' + Values[21] + '&consult_id=' + Values[0] + '&accountSearchID=' + orgid + '&jdId=' + Values[19] + '&accountFlag=' + accountFlag + '&techReviewStatus=' + Values[5] + '&vendor='+vendor+">Click</td>"));
               // }
                if(csrRoleId == 1 || vendor == 'yes'){
                   	if(Values[20] == "null"){
 ...
 $('#consultantListTable').tablesPaginate({
		navigateType : 'navigator'
	}, recordPage,consultantListTable);
}
/*
	...
	$.ajax({
        ...
        success: function(responseText) {
        	...
        	if(responseText=='Existed')
        		 window.location = 'sb/recruitment/consultant/consultDownloadAttachment.action?acc_attachment_id='+acc_attachment_id;
        	else
        		swal({
...
function doSubmittedAttachmentDownload(acc_attachment_id)
{
	$.ajax({
        url:CONTENXT_PATH + "/recruitment/consultant/checkResumeExistOrNot.action?acc_attachment_id="+acc_attachment_id,//
        context: document.body,
...
function displayConsultantList(resText){
    ...
    var reqSkill = encodeURIComponent(document.getElementById('reqSkill').value);
   // var reqSkill = $('#reqSkillSet').val()
    //var jobTitle = $('#jobTitle').val()
    var jobTitle = encodeURIComponent(document.getElementById('jobTitle').value);
    var maxRate = $('#requirementMaxRate').val()
    var consultantAddFlag ="yes";
    ...
    for (var key in consultantJson) {
    	if (consultantJson.hasOwnProperty(key)) {
           ...
           // row.append($('<td><a
            ...
            // row.append($("<td> <a
			// href='javascript:getConsultantFullDetails("+empId+");'
			// onclick='getConsultantFullDetails("+empId+");'
			// data-toggle='modal'
			// data-target='#consultantOverlay'>"+val["NAME"]+"</a> </td>"));
              row.append($('<td><a href="../../recruitment/consultantLogin/getConsultantProfile.action?jobTitle=' + jobTitle +'&orgid=' + orgid +'&vendor=' + vendor +'&reqSkill=' + reqSkill +'&maxRate=' + maxRate +'&targetRate=' + targetRate +'&requirementId=' + requirementId +'&jdId=' + jdId +'&consultantAddFlag=' +consultantAddFlag+'&consult_id=' + empId + '&accountFlag='+roleFlag +'&consultStatus=">'+val["NAME"]+"</a></td>"));
            ...
            // row.append($("<td><a
			// href='javascript:submitConsulatntProfileToRequirement("+empId+","+reqId+")'>Submit</a></td>"));
              row.append($('<td><a href="#" class="addAuthAccOverlay_popup_open" onclick="setUserAndRequirementValuesToOverlay(' + empId + ',' + reqId + ');">Submit</a></td>'));
...
function setUserAndRequirementValuesToOverlay(empId,reqId){
  // alert(empId);
  document.getElementById("overlayUserId").value=empId;
  document.getElementById("overlayRequirementId").value=reqId;
  if(isReqHaveSecurityDepositPolicy(reqId)){
  securityDepositOverlayFun();
  document.getElementById("securityDepositConsultantSubmitBtn").style.display = "block";
  document.getElementById("addConSubmit").disabled = true;
  document.getElementById("securityDepositAgree").checked = false;
  }
  else{
	  submitConsulatntProfileToRequirement();
  }
}


function isReqHaveSecurityDepositPolicy(reqId){
	var isExist=false;
	
	   var url=CONTENXT_PATH + '/recruitment/consultant/getSecurityDepositDetails.action?requirementId='+reqId;
...
function securityDepositAggrement(){
if (document.getElementById('securityDepositAgree').checked) 
{
	document.getElementById("addConSubmit").disabled = false;
} else {
	document.getElementById("addConSubmit").disabled = true;
}
}

function submitConsulatntProfileToRequirement() {
	
	var consultantId=$("#overlayUserId").val();
    var requirementId=$("#overlayRequirementId").val();
    // alert(consultantId);
    // alert(requirementId);
...
function consultantListGridDisplay(response) { 
   ...
   // var reqSkill = $('#reqSkillSet').val()
    ...
    var RequirementTaxTerm = $("#RequirementTaxTerm").val();
    var consultantAddFlag ="yes";
    //alert(orgid+" "+targetRate+" "+ maxRate+" "+vendor+" "+reqSkillSet+" "+jobTitle);
    // alert(RequirementTaxTerm) 
    if (response != null)
        var eduList = response.split("^");
 ...
 if (response.length > 0) {
    ...
    $("#consultantListTable").append(row); // this will append tr element to
                ...
                var v_comments = Values[15].trim().replace(/ /g, '%20');
                   ...
                   // row.append($('<td><a
					// href=../consultant/getConsultantDetails.action?vendor=' +
					// vendor + '&consult_id=' + Values[0] + '&consultFlag=' +
					...
					// '&vendorcomments=' + v_comments + ">" + Values[1] +
					// "</a></td>"));
                row.append($('<td><a href=../../recruitment/consultantLogin/getConsultantProfile.action?orgid='+ orgid +'&jobTitle='+ jobTitle +'&consultantAddFlag='+consultantAddFlag+'&targetRate='+targetRate+'&maxRate='+maxRate+'&vendor='+vendor+'&reqSkill='+reqSkill+'&consult_id=' + Values[0] + '&requirementId=' + requirementId + '&jdId='+ jdId +'&accountFlag='+roleFlag+'&consultStatus='+Values[5]+">"+Values[1]+"</a></td>"));
 ...
 * 
 */
function uploadBackgroundAttachment(consultantId,reqId,orgid,pageFlag,vendor){
	...
	$("#current_orgid").val(orgid);
	$("#current_pageFlag").val(pageFlag);
	$("#current_vendor").val(vendor);
...
function backgroundAttachmentValidation() {
	...
	if ($("#backgroundAttachmentTitle").val().trim() == '') {

		$("#backgroundAttachmentmessage").html(" <font color='red'>Please enter Attachment Title</font>").show().delay(3000).fadeOut();
		return false;
	}
	else if ($("#backgroundAttachmentType").val() == '0') {

		$("#backgroundAttachmentmessage").html(" <font color='red'>Please select Attachment Type</font>").show().delay(3000).fadeOut();
		return false;
	} 
	// To check if user upload any file
	else if (FileUploadPath == '') {

		$("#backgroundAttachmentmessage").html(" <font color='red'>Please upload a file</font>").show().delay(3000).fadeOut();
		return false;
	}else {
		...
		if (Extension == "doc" || Extension == "docx") {
			...
			$("#backgroundAttachmentmessage").html("<font color='red'> Allows only doc or docx extension.</font>").show().delay(3000).fadeOut();
			return false;
		}
	}
}
...
function updateBackgroundStatus(consultantId,reqId,attachmentId,status,vStatus,vComments,roleId,orgid,pageFlag,vendor){
	// alert(consultantId +"--"+reqId+"--"+attachmentId);
	$("#ucurrent_orgid").val(orgid);
	$("#ucurrent_pageFlag").val(pageFlag);
	$("#ucurrent_vendor").val(vendor);
	
	if(status=="Verified")
	{
		if(vStatus=="Reliable")
		{
		document.getElementById("verificationStatusAndCommentsDiv").style.display='block';
		document.getElementById("vStatusDiv").style.display='block';
		document.getElementById("vCommentsDiv").style.display='none';
		$("#vStatus").val(vStatus);
		}
		else
		{
	document.getElementById("verificationStatusAndCommentsDiv").style.display='block';
	document.getElementById("vStatusDiv").style.display='block';
	document.getElementById("vCommentsDiv").style.display='block';
	$("#vStatus").val(vStatus);
	$("#vComments").val(vComments);
		}
	}
	else
		{
		document.getElementById("verificationStatusAndCommentsDiv").style.display='none';
		}
	if(status=="Initiated"||(status=="Verified" && roleId==11))
		{
		document.getElementById("backgroundStatus").options.length = 0;
		$("#backgroundStatus").append('<option value="0">Select Background Status</option>');
		$("#backgroundStatus").append('<option value="Initiated">Re-Initiate</option>');
		}
	else
		{
		document.getElementById("backgroundStatus").options.length = 0;
		$("#backgroundStatus").append('<option value="0">Select Background Status</option>');
		$("#backgroundStatus").append('<option value="Initiated">Re-Initiate</option>');
		$("#backgroundStatus").append('<option value="Verifying">Verify</option>');
		$("#backgroundStatus").append('<option value="Approved">Approve</option>');
		$("#backgroundStatus").append('<option value="Rejected">Reject</option>');
		
		}
	$("#backgroundAttachmentmessage").html("");
	$("#uCurrent_consult_id").val(consultantId);
	$("#uCurrent_requirementId").val(reqId);
	$("#uCurrent_attachmentId").val(attachmentId);
	$("#backgroundStatus").val(0);
...
{
	$("#backgroundStatusMessage").html("");
	$("#verifiedBy").val(0);
	$("#consultBackgroundAttachment").val('');
	$("#backgroundAttachmentType").val(0);
	$("#backgroundAttachmentTitle").val('');
	$("#backgroundCheckComments").val('');
	var status=document.getElementById("backgroundStatus").value;
	// alert(status);
	document.getElementById("CommentsDiv").style.display='none';
	document.getElementById("attachmentFileDiv").style.display='none';
	document.getElementById("attachmentTypeDiv").style.display='none';
	document.getElementById("attachmentTitleDiv").style.display='none';
	document.getElementById("verifiedByDiv").style.display='none';
	document.getElementById("fakeCandidateDiv").style.display='none';
	
	
	if(status=="Verifying")
		{
		document.getElementById("verifiedByDiv").style.display='block';
		}
	else if(status=="Initiated")
		{
		$("#backgroundStatusMessage").show();
		$("#backgroundStatusMessage").html("<font color='red'>If you Re-Initiate background status, the old background attachments will be In-Activated.</font>");
		setTimeout('$("#backgroundStatusMessage").hide()',5000);
		document.getElementById("attachmentFileDiv").style.display='block';
		document.getElementById("attachmentTypeDiv").style.display='block';
		document.getElementById("attachmentTitleDiv").style.display='block';
		}
	else if(status=="Rejected")
		{
		document.getElementById("CommentsDiv").style.display='block';
		
		//var v= isReqHaveSecurityDeposit();
		//alert("v vlaue-->"+v);
		if(isReqHaveSecurityDeposit()){
			document.getElementById("fakeCandidateDiv").style.display='block';
		}
		else{
			document.getElementById("fakeCandidateDiv").style.display='none';
		}
...
function isReqHaveSecurityDeposit(){
	var isExist=false;
	var requirementId=document.getElementById("uCurrent_requirementId").value;
	   var url=CONTENXT_PATH + '/recruitment/consultant/getSecurityDepositDetails.action?requirementId='+requirementId;
...
function backgroundStatusValidation() {
	$("#backgroundStatusMessage").html("");
	var backgroundStatus = document.getElementById("backgroundStatus").value;

	// To check if user upload any file
	if (backgroundStatus == '0') {

		$("#backgroundStatusMessage").html(" <font color='red'>Please Select Background Status</font>");
		return false;
	}
	else if(backgroundStatus=='Initiated')
		{
		$("#backgroundStatusMessage").html("");
		var FileUploadPath = document.getElementById('consultBackgroundAttachmentInStatus').value;

		// To check if user upload any file
		if (FileUploadPath == '') {

			$("#backgroundStatusMessage").html(" <font color='red'>Please upload a file</font>");
			return false;
		}else if ($("#updateBackgroundAttachmentType").val() == '0') {

			$("#backgroundStatusMessage").html(" <font color='red'>Please select Attachment Type</font>");
			return false;
		} else if ($("#updateBackgroundAttachmentTitle").val().trim() == '') {

			$("#backgroundStatusMessage").html(" <font color='red'>Please enter Attachment Title</font>");
			return false;
		} else {
			var Extension = FileUploadPath.substring(
					FileUploadPath.lastIndexOf('.') + 1).toLowerCase();
	...
	else if (backgroundStatus == 'Verifying') {
		if($("#verifiedBy").val() == '0')
			{
			$("#backgroundStatusMessage").html(" <font color='red'>Please select Assigned To</font>");
			return false;
			}
		} 	
	else if (backgroundStatus == 'Rejected') {
		if($("#backgroundCheckComments").val() == ''){
			$("#backgroundStatusMessage").html(" <font color='red'>Please enter Comments</font>");
			return false;
		}
	}
}

function backgroundAddValidationInStatus() {
	document.getElementById('backgroundStatusMessage').innerHTML = '';
	var fullPath = document.getElementById('consultBackgroundAttachmentInStatus').value;

	// alert(fullPath);

	var size = document.getElementById('consultBackgroundAttachmentInStatus').files[0].size;
	var extension = fullPath.substring(fullPath.lastIndexOf('.') + 1);
	var leafname = fullPath.split('\\').pop().split('/').pop();
	if (extension == "doc" || extension == "docx") {
		var size = document.getElementById('consultBackgroundAttachmentInStatus').files[0].size;
		...
		if (leafname.length > 30) {
			document.getElementById('consultBackgroundAttachmentInStatus').value = '';
			document.getElementById('backgroundStatusMessage').innerHTML = "<font color=red>File name length must be less than 30 characters!</font>"
			// showAlertModal("File size must be less than 2 MB");
			return (false);
		} else {
			if (parseInt(size) < 2097152) {
				...
				document.getElementById('consultBackgroundAttachmentInStatus').value = '';
				document.getElementById('backgroundStatusMessage').innerHTML = "<font color=red>File size must be less than 2 MB.</font>"
				// showAlertModal("File size must be less than 2 MB");
				return (false);
			}
		}
		...
		document.getElementById('consultBackgroundAttachmentInStatus').value = "";
		document.getElementById('backgroundStatusMessage').innerHTML = "<font color=red>Invalid file extension!Please select doc or docx  file.</font>"
		return false;
	}
}
function updateBgStatusByCSR(consultantId,reqId,attachmentId,orgid,pageFlag,vendor){
	// alert("hello");
	// alert(consultantId +"--"+reqId+"--"+attachmentId);
	$("#bgStatusByCSRMessage").html("");
	$("#vcurrent_consult_id").val(consultantId);
	$("#vcurrent_requirementId").val(reqId);
	$("#vcurrent_attachmentId").val(attachmentId);
	$("#vcurrent_orgid").val(orgid);
	$("#vcurrent_pageFlag").val(pageFlag);
	$("#vcurrent_vendor").val(vendor);
...
{
	$("#bgStatusByCSRMessage").html("");
	var status=document.getElementById("verificationStatus").value;
	$("#verificationComments").val("");
	
	document.getElementById("verificationCommentsDiv").style.display='none';
	
	if(status=="Non-Reliable")
		{
		document.getElementById("verificationCommentsDiv").style.display='block';
		}
}
function viewVerificatonStatus(vStatus,vComments,comments,status){
	// alert(consultantId +"--"+reqId+"--"+attachmentId);
	$("#viewVerificationStatus").val(vStatus);
	$("#viewVerificationComments").val(vComments);
	$("#viewCustorVenComments").val(comments);
	
	if(vStatus=="Non-Reliable")
	{
		document.getElementById("viewVerificationCommentsDiv").style.display='block';	
		
	}
	else
		{
		document.getElementById("viewVerificationCommentsDiv").style.display='none';
		}
	if(status=="Rejected")
		{
		document.getElementById("viewCustorVenCommentsDiv").style.display='block';
		}
	else
		{
		document.getElementById("viewCustorVenCommentsDiv").style.display='none';
		}
	}
function SendReqNotificationToCSR()
{
	var securityDepositCheck="no";
	var reqId=document.getElementById("overlayReqId").value;
	var orgId=document.getElementById("overlayOrgId").value;
	
	
	var overlaySecurityDepositList=document.getElementById("overlaySecurityDepositList").value;
	var profileSubmissionCost=document.getElementById("overlayProfileSubmissionCost").value;
	var profileSelectedCost=document.getElementById("overlayProfileSelectedCost").value;
	var profileRejectionCost=document.getElementById("overlayProfileRejectionCost").value;
	var fakeCandidateFine=document.getElementById("overlayFakeCandidateFine").value;
	var comments=document.getElementById("overlayComments").value;
	if(document.getElementById("securityDepoCheck1").checked){
		
		securityDepositCheck="yes";
		if(overlaySecurityDepositList=="-1"){
			document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Select Security Deposit</font>";
			return false;
		}
	
	if(profileSubmissionCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Submission Cost</font>";
		return false;
	}
	if(profileSelectedCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Selection Cost</font>";
		return false;
	}
	if(profileRejectionCost.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Profile Rejection Cost</font>";
		return false;
	}
	if(fakeCandidateFine.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Fake Candidate Fine</font>";
		return false;
	}
	if(comments.trim()==""){
...
//	alert("securityDepositCheck-->"+securityDepositCheck);
	swal({
		 title: "Do you want to release requirement?",
		textSize:"170px",
	    type: "warning",
	    showCancelButton: true,
	    confirmButtonColor: "#3498db",
		...
		function(isConfirm){
			if (isConfirm) {
			 $.ajax({
			        url:"SendReqNotificationToCSR.action?requirementId="+reqId+"&orgId="+orgId+"&profileSubmissionCost="+profileSubmissionCost+"&profileSelectedCost="+profileSelectedCost+"&profileRejectionCost="+profileRejectionCost+"&fakeCandidateFine="+fakeCandidateFine+"&securityDepositCheck="+securityDepositCheck+"&comments="+comments,//
			        async: false,
			        context: document.body,
			        success: function(responseText) {
			        	// alert("responseText-->"+responseText)
			        	 swal("Success", "Released Successfully  ", "success");
			        	// getSearchRequirementsList(1);
			       }, 
			        error: function(e){
			            alert("error-->"+e);
...
function getReleaseVendorsForCsr(requirementId,orgId,taxTerm){
	 window.location= "/sb/Requirements/getReleaseVendors.action?requirementId="+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm;
}
...
var tier= document.getElementById("tier").value;
 ...
 var url='./getVendorsBasedOnReqCategoryForCSR.action?requirementId='+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm+'&vendorName='+vendorName+'&vendorTierType='+tier;
var req=initRequest(url);
 req.onreadystatechange = function() {
	 document.getElementById('loadingVendorSearch').style.display = 'block';
     if (req.readyState == 4) {
         if (req.status == 200) {
        	 $('#loadingVendorSearch').hide();
             document.getElementById("requirementId").value = requirementId;
...
function populateReleaseVendorsListForCSR(data){
	// alert(data);
	 var vendorsIdString='';
         ...
         if (flag.length > 0) {
	        ...
	        for (var i = 0; i < addList.length-1; i++) {
               ...
               cell = document.createElement("TD");
	            ...
	            $("#releaseReqTable").append(row); // this will append tr
													// element to table... keep
													// its reference for a while
													// since we will add cels
													// into it
	    ...
	    pagerOption();
	    /*
		 * pag.init(); pag.showPageNav('pager', 'pageNavPosition');
		 * pag.showPage(1);
		 */
}
	...
	if (check ==true){
		...
		// alert("vendorsIdString.."+vendorsIdString)
		 var vendorIds=vendorsIdString.split("^^");
		    var vendorOrgIds='';
		    for(var i =0;i<vendorIds.length-1;i++){
		    	document.getElementById("vendor"+vendorIds[i]).checked=true;
		        $("#vendor"+vendorIds[i]).attr("disabled", false);
		    }
	}
	else{
...
function getSecurityDepositDetails(flag){
	// alert(flag);
	if(flag=="onclick"){
	securityDepositOverlayFun();
	}
	if(flag=="onclick" || flag=="onload"){
		document.getElementById("securityDepositConsultantSubmitBtn").style.display = 'none';
	}
	else{
		document.getElementById("securityDepositConsultantSubmitBtn").style.display = 'block';
	}
	          ...
	          // alert(req.responseText);
	           if((req.responseText)==""){
	        	 ...
	        	 var Values = (req.responseText).split("|");
	  	       	 ...
	  	       	 document.getElementById("overlayProfileSubmissionCost").value=Values[0];
	  	    	 document.getElementById("overlayProfileRejectionCost").value=Values[1];
	  	    	 document.getElementById("overlayFakeCandidateFine").value=Values[2];
	  	    	 document.getElementById("overlayComments").value=Values[3];
	  	    	document.getElementById("overlayProfileSelectedCost").value=Values[4];
	  	    	 document.getElementById("securityDepositViewBtn").style.display="block";
...
function securityDepositOverlayFun() {
...
function enableOrDisableSecurityDepo(securityDepoCheck){
	// alert(securityDepoCheck);
	if(securityDepoCheck=="1"){
		document.getElementById("securityDepoDiv").style.display="block";
	}
	else{
		document.getElementById("securityDepoDiv").style.display="none";
		document.getElementById("validationMessage").innerHTML="";
	}
}
function removeDateErrorMsg()
{
    // alert("hello jagan")
    $("#submissionValidation").html("");
    $("#submittedEnd").css('border','1px solid #ccc');
    $("#submittedFrom").css('border','1px solid #ccc');
